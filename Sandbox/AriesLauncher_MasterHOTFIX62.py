...
class VAApp(ctk.CTk):
    ...
    def _browser_clear_entry(self):
        ...
        if self._browser_entry is not None:
            try:
                ...
                pass
        self._active_input = self._browser_entry
        self._active_submit = self._browser_launch
        self.toast.show("Browser field cleared", 700)
    ...
    def show_browser(self, search_query=None):
        ...
        self._btn(nav, "Open", go, True, 52 if compact else 72)
        self._btn(nav, "Clear", self._browser_clear_entry, w=52 if compact else 72)
        self._btn(nav, "Home", self._browser_open_home, w=52 if compact else 72)
        self._btn(nav, "Type", self._browser_show_type_keyboard, w=52 if compact else 72)
        self._btn(nav, "Close", lambda: (self._close_browser(), self._go_home()), w=52 if compact else 72)
