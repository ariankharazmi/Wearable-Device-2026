...
class CoverFlow:
    def __init__(self, canvas, apps):
        ...
        self._label = canvas.create_text(
            0, 0, text="", fill=TXT, font=(FONT, 17))
...
class VAApp(ctk.CTk):
        ...
        try:
            ...
            for output_name in outputs:
                ...
                if result.returncode == 0:
                    applied = True
            if applied:
                self._last_brightness_mode = "xrandr"
    ...
    def _adjust_volume(self, delta):
        ...
        try:
            result = subprocess.run(["amixer", "sset", "Master", step], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
            if result.returncode == 0:
                self._last_volume_mode = "alsa Master"
        ...
        if self._volume_target:
            ...
            try:
                result = subprocess.run(["amixer", "-c", card, "sset", name, step], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
                if result.returncode == 0:
    ...
    def _cam_tick(self):
        ...
        try:
            ...
            if ret:
                ...
                preview_size = (560, 255) if (WIDTH <= 640 or HEIGHT <= 400) else (980, 560)
    ...
    def show_photos(self):
        ...
        for i, fname in enumerate(files):
            ...
            try:
                ...
                ctk.CTkButton(row, image=ctk_img, text="", width=thumb_btn[0], height=thumb_btn[1],
    ...
    def show_settings(self):
        ...
        light_btn = ctk.CTkButton(row1, text="Light", command=lambda: self._set_system_light_mode(True),
                                  ...
                                  hover_color="#3A3A3C", text_color=button_text, width=48 if compact else 120)
        ...
        wifi_btn = ctk.CTkButton(row1, text="WiFi", command=self.show_wifi,
                                 corner_radius=14 if compact else 20, height=26 if compact else 32,
                                 font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                 hover_color="#3A3A3C", text_color=button_text, width=48 if compact else 120)
        wifi_btn.pack(side="left", padx=2 if compact else 4)
        ...
        bri_minus_btn = ctk.CTkButton(brow, text="Bri-", command=lambda: (self._adjust_alpha(-0.05), self.after(80, self._refresh_settings_status)),
                                      corner_radius=14 if compact else 20, height=26 if compact else 32,
                                      font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                      hover_color="#3A3A3C", text_color=button_text, width=42 if compact else 120)
        ...
        vol_minus_btn = ctk.CTkButton(vrow, text="Vol-", command=lambda: (self._adjust_volume(-1), self.after(120, self._refresh_settings_status)),
                                      corner_radius=14 if compact else 20, height=26 if compact else 32,
                                      font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                      hover_color="#3A3A3C", text_color=button_text, width=42 if compact else 100)
        ...
        ctk.CTkLabel(p, text=f"Microphone — {mic_state}",
                      ...
                      text_color=TXT).pack(anchor="w", padx=padx)

        mic_box = ctk.CTkTextbox(p, height=50 if compact else 80, font=(FONT, 11 if compact else 11),
                                  fg_color="#1C1C1E", text_color=TXT)
        mic_box.pack(fill="x", padx=padx, pady=(1, 1))
        ...
        if not mics:
            ...
            for i, name in mics:
                ...
                if self.voice.mic_index is None and i == 0:
                    tag = " < DEFAULT"
                short = name if not compact else (name[:24] + "…" if len(name) > 24 else name)
                mic_box.insert("end", f"  [{i}] {short}{tag}\n")
        ...
        if self.rotary.available:
            ...
            ctk.CTkLabel(p, text="Rotary Encoder — Connected",
                          font=(FONT, 11 if compact else 12), text_color=GRN).pack(anchor="w", padx=padx)
    ...
    def _query_volume_pct(self):
        try:
            out = subprocess.check_output(["amixer", "sget", "Master"], stderr=subprocess.DEVNULL, text=True)
            m = re.search(r"\[(\d{1,3})%\]", out)
            if m:
                return int(m.group(1))
        except Exception:
            pass
        target = getattr(self, "_volume_target", None)
        if target:
            card, name = target
            try:
                out = subprocess.check_output(["amixer", "-c", card, "sget", name], stderr=subprocess.DEVNULL, text=True)
                m = re.search(r"\[(\d{1,3})%\]", out)
                if m:
                    return int(m.group(1))
            except Exception:
