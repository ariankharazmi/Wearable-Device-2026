...
class VAApp(ctk.CTk):
    ...
    _HOME_NOISE_FLOOR = 10

    def _home_touch_track(self, x, y):
        """Track relative finger travel like an Apple TV remote strip.

        We accumulate small deltas from the previous sample rather than mapping
        absolute X or using a far-away anchor. That makes short swipes register
        sooner and avoids the jump/ignore pattern from HOTFIX16.
        """
        now = time.perf_counter()
    ...
    def _touchpad_tap(self):
        ...
        if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.34:
            return
        if (now - getattr(self, "_last_touchpad_motion_at", 0.0)) < 0.16:
            return
