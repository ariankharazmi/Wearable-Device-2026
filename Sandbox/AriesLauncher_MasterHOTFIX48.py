...
class VAApp(ctk.CTk):
    ...
    def _capture_photo(self):
        ...
        if frame is None and self._cam_cap:
            try:
    ...
    def show_browser(self, search_query=None):
        ...
        top.pack(fill="x")
        top.pack_propagate(False)
        ctk.CTkLabel(top, text="â€¹  VA-OS 1.4.13.26 Browser", font=(FONT, 18), text_color=C).pack(side="left", padx=24, pady=12)
        ...
        self._btn(nav, "⌨️", self._browser_show_type_keyboard, w=54)
        self._btn(nav, "←", self._browser_go_back, w=54)
        self._btn(nav, "→", self._browser_go_forward, w=54)
        ...
        help_row = ctk.CTkLabel(
            shell,
            text="Rotate = Prev/Next focus  ·  Click = activate  ·  Long press = exit  ·  ⌨️ opens keyboard",
            font=(FONT, 11),
            text_color=TXTD,
            justify="center",
        )
        ...
        self.after(120, lambda: self._focus_root())
        if search_query:
            self.after(180, go)
    ...
    def show_translate(self):
        ...
        self._btn(top, "Trans", lambda: self._translate_target_button(), w=72)
    ...
    def show_settings(self):
        ...
        dm = ctk.CTkSwitch(p, text="Dark mode", **sw,
                            command=lambda: self._set_system_light_mode(False if dm.get() else True))
        dm.select(); dm.pack(anchor="w", padx=20, pady=(12, 6))

        lm = ctk.CTkSwitch(p, text="System light mode", **sw,
                            command=lambda: self._set_system_light_mode(bool(lm.get())))
        if not self.dark_mode:
            lm.select()
        lm.pack(anchor="w", padx=20, pady=6)
        ...
        ctk.CTkLabel(p, text="Brightness", font=(FONT, 12),
                      text_color=TXTD).pack(anchor="w", padx=20, pady=(12, 2))
        bsl = ctk.CTkSlider(
            p,
            from_=30, to=100, number_of_steps=14, **sl,
            command=lambda v: self._adjust_alpha((float(v)/100.0) - getattr(self, "_display_brightness", 0.92))
        )
        bsl.set(int(getattr(self, "_display_brightness", 0.92) * 100))
        bsl.pack(fill="x", padx=20)

        brow = ctk.CTkFrame(p, fg_color="transparent")
        brow.pack(fill="x", padx=20, pady=(6, 0))
        self._btn(brow, "Brightness -", lambda: self._adjust_alpha(-0.05), w=110)
        self._btn(brow, "Brightness +", lambda: self._adjust_alpha(+0.05), w=110)

        ctk.CTkLabel(p, text="Volume", font=(FONT, 12),
                      text_color=TXTD).pack(anchor="w", padx=20, pady=(12, 2))
        vsl = ctk.CTkSlider(
            p,
            from_=0, to=100, number_of_steps=20, **sl,
            command=lambda v: None
        )
        vsl.set(50)
        vsl.pack(fill="x", padx=20)

        vrow = ctk.CTkFrame(p, fg_color="transparent")
        vrow.pack(fill="x", padx=20, pady=(6, 0))
        self._btn(vrow, "Volume -", lambda: self._adjust_volume(-1), w=110)
        self._btn(vrow, "Volume +", lambda: self._adjust_volume(+1), w=110)
