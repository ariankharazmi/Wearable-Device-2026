...
class VAApp(ctk.CTk):
    ...
    def show_browser(self, search_query=None):
        ...
        self._btn(nav, "Open", go, True, 58 if compact else 72)
        self._btn(nav, "Home", self._browser_open_home, w=58 if compact else 72)
        self._btn(nav, "Type", self._browser_show_type_keyboard, w=58 if compact else 72)
        self._btn(nav, "Close", lambda: (self._close_browser(), self._go_home()), w=58 if compact else 72)
