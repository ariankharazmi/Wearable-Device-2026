...
class VAApp(ctk.CTk):
    ...
    def _find_volume_target(self):
        try:
            ...
            for name in ("Master", "PCM", "Speaker", "Headphone", "Playback", "Digital"):
                if f"'{name}'" in out:
                    return (None, name)
        except Exception:
            pass
        preferred_cards = ["0", "1", "2", "3"]
        preferred_names = ["Master", "PCM", "Speaker", "Headphone", "Playback", "Digital"]
    ...
    def _adjust_volume(self, delta):
        ...
        for cmd, mode in ((["wpctl", "set-volume", "@DEFAULT_AUDIO_SINK@", "5%+" if delta > 0 else "5%-"], "wpctl"),
                          (["pactl", "set-sink-volume", "@DEFAULT_SINK@", "+5%" if delta > 0 else "-5%"], "pactl")):
            try:
                result = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
                if result.returncode == 0:
                    self._last_volume_mode = mode
