...
class TouchKeyboard:
    ...
    def ensure(self):
        ...
        for col in range(self.VISIBLE_SLOTS):
            btn = ctk.CTkButton(
                ...
                height=34 if WIDTH <= 640 or HEIGHT <= 400 else 40,
                corner_radius=9,
                border_width=1,
                border_color="#2A2A2E",
                font=(FONT, 12, "bold"),
    ...
    def _position(self, force=False):
        ...
        abs_h = 94 if (WIDTH <= 640 or HEIGHT <= 400) else (146 if app_h <= 420 else 156)
...
class VAApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        if WIDTH <= 640 or HEIGHT <= 400:
            try:
                ctk.set_widget_scaling(0.68)
                ctk.set_window_scaling(0.68)
                self.tk.call("tk", "scaling", 0.68)
            except Exception:
                pass
    ...
    def _handle_rotary_click(self):
        ...
        if self.current_view == "browser" and self._browser_is_open() and not self._touch_keyboard.visible:
            if (now - getattr(self, "_last_rotary_click_at", 0.0)) <= 0.45:
    ...
    def _view(self, vid, title, body="", placeholder=False):
        ...
        top = ctk.CTkFrame(self._vf, fg_color="transparent", height=34 if compact else 52)
        top.pack(fill="x")
        top.pack_propagate(False)
        ctk.CTkLabel(top, text=f"‹  {title}", font=(FONT, 13 if compact else 18),
                      text_color=C).pack(side="left", padx=10 if compact else 24, pady=7 if compact else 12)
        ctk.CTkLabel(top, text="Esc back",
                      font=(FONT, 7 if compact else 10),
        ...
        if body:
            ctk.CTkLabel(self._vf, text=body, font=(FONT, 8 if compact else 11),
        ...
        if placeholder:
            ctk.CTkLabel(panel, text=f"{title} — coming soon",
                          font=(FONT, 9 if compact else 14), text_color=TXTD,
    ...
    def show_camera(self):
        ...
        self._cam_label = ctk.CTkLabel(p, text="Initializing …",
                                        font=(FONT, 9 if (WIDTH <= 640 or HEIGHT <= 400) else 13), text_color=TXTD)
    ...
    def show_browser(self, search_query=None):
        ...
        ctk.CTkLabel(top, text="Long press exits", font=(FONT, 7 if compact else 10), text_color="#48484A").pack(side="right", padx=8 if compact else 18, pady=5 if compact else 10)
        ctk.CTkFrame(self._vf, fg_color="#2C2C2E", height=1).pack(fill="x", padx=0)
        ...
        ent = ctk.CTkEntry(
            ...
            font=(FONT, 8 if compact else 13),
            fg_color=PNL,
            text_color=TXT,
            border_color=C,
        ...
        help_row = ctk.CTkLabel(
            shell,
            text=("Rotate=focus · Click=open · 2x=keys" if compact else "Rotate = focus  ·  Click = activate  ·  Double-click = keyboard"),
            font=(FONT, 6 if compact else 10),
            text_color=TXTD,
            justify="center",
        )
    ...
    def show_translate(self):
        ...
        src = ctk.CTkTextbox(p, height=(62 if (WIDTH <= 640 or HEIGHT <= 400) else 120), font=(FONT, 9 if (WIDTH <= 640 or HEIGHT <= 400) else 13),
                              fg_color="#1C1C1E", text_color=TXT)
        src.pack(fill="x", padx=10, pady=(8, 4))
        src.insert("end", "Hello, how are you?")
        src.focus_set()

        tgt = ctk.CTkTextbox(p, height=(62 if (WIDTH <= 640 or HEIGHT <= 400) else 120), font=(FONT, 9 if (WIDTH <= 640 or HEIGHT <= 400) else 13),
    ...
    def show_settings(self):
        ...
        ctk.CTkLabel(p, textvariable=self._settings_brightness_var,
                      font=(FONT, 9 if compact else 13, "bold"), text_color=TXT).pack(anchor="w", padx=padx, pady=(3, 1))
        ...
        ctk.CTkLabel(p, text=f"Microphone — {mic_state}",
                      font=(FONT, 9 if compact else 14, "bold"),
                      text_color=TXT).pack(anchor="w", padx=padx)

        mic_box = ctk.CTkTextbox(p, height=28 if compact else 80, font=(FONT, 7 if compact else 11),
                                  fg_color="#1C1C1E", text_color=TXT)
        ...
        if self.rotary.available:
            ...
            ctk.CTkLabel(p, text="Rotary Encoder — Connected",
                          font=(FONT, 8 if compact else 12), text_color=GRN).pack(anchor="w", padx=padx)
