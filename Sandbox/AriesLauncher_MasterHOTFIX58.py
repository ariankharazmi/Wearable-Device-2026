...
class TouchKeyboard:
    ...
    def _tokens(self):
        ...
        if getattr(self.app, "current_view", "") == "browser" and getattr(self.app, "_browser_typing_mode", False):
            # Put ← → next to CAPS on the Apple-TV keyboard instead of at the front.
            if self.TOK_CAPS in base:
                idx = base.index(self.TOK_CAPS) + 1
                base[idx:idx] = [self.TOK_BROWSER_BACK, self.TOK_BROWSER_FORWARD]
            elif self.TOK_LETTERS in base:
                idx = base.index(self.TOK_LETTERS)
                base[idx:idx] = [self.TOK_BROWSER_BACK, self.TOK_BROWSER_FORWARD]
            else:
                base += [self.TOK_BROWSER_BACK, self.TOK_BROWSER_FORWARD]
...
class VAApp(ctk.CTk):
    ...
    def _browser_show_type_keyboard(self):
        if not self._browser_can_remote_control():
            self.toast.show("Install xdotool: sudo apt install xdotool", 2500)
            return
        self._browser_typing_mode = True
        # Register the URL entry as active_input so _show_touch_keyboard works.
        # This is the HOTFIX46 approach that made the browser keyboard work.
        ...
        self._touch_keyboard.refresh()
        self.toast.show("← → navigate  Enter=open  rotate=elements  2×=close", 2000)
