...
class VAApp(ctk.CTk):
    ...
    _KEYBOARD_DRAG_START_PX = 320.0
    _KEYBOARD_STEP_PX = 360.0

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 380.0
    _HOME_DRAG_START_PX = 235.0
    ...
    def _touchpad_tap(self):
        ...
        if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.26:
            return
        if (now - getattr(self, "_last_touchpad_motion_at", 0.0)) < 0.14:
            return
