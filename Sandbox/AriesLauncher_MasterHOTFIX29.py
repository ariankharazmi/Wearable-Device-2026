...
class VAApp(ctk.CTk):
    ...
    def _touchpad_move(self, x, y):
        ...
        try:
            ...
            dy = y - getattr(self, "_keyboard_drag_last_y", y)
            self._keyboard_drag_last_x = x
            self._keyboard_drag_last_y = y

            if self._keyboard_axis_lock is None:
                abs_dx = abs(dx)
                abs_dy = abs(dy)
                if abs_dx <= self._KEYBOARD_NOISE_FLOOR and abs_dy <= self._KEYBOARD_NOISE_FLOOR:
                    return
                if abs_dx >= max(self._KEYBOARD_NOISE_FLOOR + 2, abs_dy * self._AXIS_LOCK_RATIO):
                    self._keyboard_axis_lock = "x"
                elif abs_dy >= max(self._KEYBOARD_NOISE_FLOOR + 2, abs_dx * self._AXIS_LOCK_RATIO):
                    self._keyboard_axis_lock = "y"
                else:
                    return

            if self._keyboard_axis_lock != "x":
                return
    ...
    _KEYBOARD_NOISE_FLOOR = 14
    _KEYBOARD_DRAG_START_PX = 80.0     # small deadzone before keyboard drag registers (~1.7mm)
    _KEYBOARD_STEP_PX = 200.0          # ~4mm per letter — fine enough for keyboard navigation

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 550.0              # ~11mm per app step; full pad (~38mm) = ~3 apps max
    _HOME_DRAG_START_PX = 80.0         # ~1.7mm deadzone before drag registers (was 440 = ~9mm, way too stiff)
    ...
    def _home_touch_track(self, x, y):
        ...
        dy = y - getattr(self, "_home_drag_last_y", y)
        self._home_drag_last_x = x
        self._home_drag_last_y = y
        self._home_drag_last_move_at = now
        ...
        if self._home_axis_lock is None:
            abs_dx = abs(dx)
            abs_dy = abs(dy)
            if abs_dx <= self._HOME_NOISE_FLOOR and abs_dy <= self._HOME_NOISE_FLOOR:
                ...
                return
            if abs_dx >= max(self._HOME_NOISE_FLOOR + 4, abs_dy * self._AXIS_LOCK_RATIO):
                self._home_axis_lock = "x"
            elif abs_dy >= max(self._HOME_NOISE_FLOOR + 4, abs_dx * self._AXIS_LOCK_RATIO):
                self._home_axis_lock = "y"
            else:
                return

        if self._home_axis_lock != "x":
            return
    ...
    def _touchpad_tap(self):
        ...
        if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.22:
            return
