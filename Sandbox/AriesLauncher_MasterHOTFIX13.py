...
class TouchpadController:
    """Cirque GlidePoint touchpad with direct-touch style callbacks."""

    POLL_SEC = 0.004
    TAP_WINDOW_SEC = 0.18
    TAP_MAX_TRAVEL = 18
    RELEASE_TIMEOUT_SEC = 0.008
    JITTER_FLOOR = 1
    JUMP_THRESHOLD = 1800
    SWIPE_DISTANCE = 48
    SWIPE_DEBOUNCE_SEC = 0.04
    ...
    def __init__(
        ...
        self._on_touch_start = on_touch_start
        self._on_touch_end = on_touch_end
    ...
    def _handle_touch_point(self, x, y, now):
        ...
        direction = None
        if abs(total_dx) >= max(self.SWIPE_DISTANCE, abs(total_dy) + 8):
            direction = "right" if total_dx > 0 else "left"
        elif abs(total_dy) >= max(self.SWIPE_DISTANCE, abs(total_dx) + 8):
    ...
    def _finish_touch_if_needed(self, now):
        ...
        duration = now - self._touch_started
        was_tap = (
            not self._swiped_this_touch
            and duration <= self.TAP_WINDOW_SEC
            and self._max_travel <= self.TAP_MAX_TRAVEL
        )

        last_x, last_y = self._last_x, self._last_y

        if was_tap and self._on_tap:
            self._on_tap()

        if self._on_touch_end:
            self._on_touch_end(
                was_tap=was_tap,
...
class VAApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self._touch_session_mode = None
        self._touch_origin_x = None
        self._touch_origin_y = None
        self._touch_origin_sel = 0
        self._touch_drag_moved = False
        self.touchpad = TouchpadController(
            ...
            on_touch_start=lambda x, y: self.after(0, lambda tx=x, ty=y: self._touchpad_touch_start(tx, ty)),
            on_touch_end=lambda **kw: self.after(0, lambda data=kw: self._touchpad_touch_end(**data)),
        )
    ...
    def _touchpad_touch_start(self, x, y):
        self._touch_drag_moved = False
        self._touch_origin_x = x
        self._touch_origin_y = y
        if self._touch_keyboard.visible:
            self._touch_session_mode = "keyboard"
            self._touch_origin_sel = self._touch_keyboard.sel_col
        elif self.current_view == "home":
            self._touch_session_mode = "launcher"
            self._touch_origin_sel = self.cflow.sel
        else:
            self._touch_session_mode = "generic"
            self._touch_origin_sel = 0

    def _touchpad_touch_end(self, was_tap=False, max_travel=0, duration=0.0, x=None, y=None):
        if self._touch_session_mode in {"launcher", "keyboard"} and max_travel >= 20:
    ...
    def _touchpad_swipe(self, direction):
        # Home launcher and keyboard use direct drag/position tracking now.
        if self._touch_keyboard.visible or self.current_view == "home":
            return
        self._last_touchpad_swipe_at = time.perf_counter()
    ...
    def _touchpad_move(self, x, y):
        if self._touch_keyboard.visible:
            if self._touch_origin_x is None or self._touch_session_mode != "keyboard":
                self._touchpad_touch_start(x, y)
            dx = x - self._touch_origin_x
            step = 140.0
            target = int(round(self._touch_origin_sel + (dx / step)))
            target = max(0, min(len(self._touch_keyboard.TOKENS) - 1, target))
            if target != self._touch_keyboard.sel_col:
                self._touch_drag_moved = True
                self._touch_keyboard.sel_col = target
                self._touch_keyboard.refresh()
            return

        if self.current_view == "home":
            if self._touch_origin_x is None or self._touch_session_mode != "launcher":
                self._touchpad_touch_start(x, y)
            dx = x - self._touch_origin_x
            step = 170.0
            target = int(round(self._touch_origin_sel + (dx / step)))
            target = max(0, min(len(self.cflow.apps) - 1, target))
            if target != self.cflow.sel:
                self._touch_drag_moved = True
                self.cflow.sel = target
            return

    def _touchpad_tap(self):
        if (time.perf_counter() - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.14:
            return
        if self._touch_drag_moved:
            return
    ...
    def _video_capture_candidates(self):
        nodes = []
        for name in sorted([n for n in os.listdir("/dev") if n.startswith("video")]):
            path = os.path.join("/dev", name)
            info = self._video_node_info(path)
            if info.get("capture"):
                nodes.append(path)
        return nodes

    def _video_node_info(self, path):
        info = {"path": path, "capture": False, "card": None, "error": None}
        ...
        try:
            ...
            try:
                ...
                card = bytes(buf[16:48]).split(b"\0", 1)[0].decode(errors="ignore")
                ...
                info["capture"] = bool(effective & (V4L2_CAP_VIDEO_CAPTURE | V4L2_CAP_VIDEO_CAPTURE_MPLANE))
                info["card"] = card or path
            finally:
                os.close(fd)
        except Exception as exc:
            info["error"] = str(exc)
    ...
    def _start_camera(self):
        ...
        if self._cam_running and self._cam_cap and self._cam_cap.isOpened():
            self.log.debug("Camera already running on source %s", self._cam_source_index)
            return
        ...
        candidates = self._video_capture_candidates()
        if not candidates:
            if os.path.exists("/dev/video0"):
                info = self._video_node_info("/dev/video0")
                detail = "not a capture device" if not info.get("capture") else (info.get("error") or "unavailable")
                self._cam_msg(f"Camera node exists but {detail}.")
                self.status.append(f"Camera node issue: /dev/video0 {detail}")
            else:
                self._cam_msg("No camera capture device found.")
                self.status.append("Camera: no /dev/video* capture node")
            return
        ...
        for source in candidates:
            ...
            try:
                ...
                if cap and cap.isOpened():
                    self._cam_cap = cap
                    self._cam_source_index = source
                    self.log.info("Camera opened on %s", source)
                    break
            except Exception:
                self.log.exception("Camera initialization failed on %s", source)
            finally:
        ...
        if not self._cam_cap or not self._cam_cap.isOpened():
            self._cam_cap = None
            self._cam_running = False
            self._cam_msg("Camera capture open failed.")
            return
