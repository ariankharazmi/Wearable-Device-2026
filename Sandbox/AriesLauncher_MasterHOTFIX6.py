...
class TouchKeyboard:
    ...
    def ensure(self):
        ...
        for r, row_tokens in enumerate(self.LAYOUT):
            ...
            for token in row_tokens:
                ...
                self._linear_tokens.append(token)
                col += span
    ...
    def _position(self, force=False):
        ...
        rel_h = 0.32 if app_h <= 480 else 0.34
        abs_h = max(190, min(235, int(app_h * rel_h)))
    ...
    def move(self, dx=0, dy=0):
        ...
        current = self.LAYOUT[self.sel_row][self.sel_col]
        try:
            idx = self._linear_tokens.index(current)
        except ValueError:
            idx = 0

        idx = max(0, min(len(self._linear_tokens) - 1, idx + delta))
        pos = self._find_token(self._linear_tokens[idx])
        if pos is not None:
