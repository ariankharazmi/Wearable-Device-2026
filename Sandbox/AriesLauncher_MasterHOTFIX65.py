...
class TouchKeyboard:
    ...
    TOK_BROWSER_SHIFT_LEFT = "__BROWSER_SHIFT_LEFT__"
    TOK_BROWSER_SHIFT_RIGHT = "__BROWSER_SHIFT_RIGHT__"
    TOK_TRANS = "__TRANS__"
    ...
    def _tokens(self):
        ...
        if getattr(self.app, "current_view", "") == "browser" and getattr(self.app, "_browser_typing_mode", False):
            ...
            base[idx:idx] = [
                ...
                self.TOK_BROWSER_SHIFT_LEFT,
                self.TOK_BROWSER_SHIFT_RIGHT,
            ]
    ...
    def _label(self, token):
        ...
        if token == self.TOK_BROWSER_FORWARD:
            return "→"
        if token == self.TOK_BROWSER_SHIFT_LEFT:
            return "Shift-L"
        if token == self.TOK_BROWSER_SHIFT_RIGHT:
            return "Shift-R"
...
class VAApp(ctk.CTk):
    ...
    def _cam_tick(self):
        ...
        try:
            ...
            if ret:
                ...
                preview_size = (500, 220) if (WIDTH <= 640 or HEIGHT <= 400) else (920, 520)
    ...
    def show_settings(self):
        ...
        self._settings_volume_var = tk.StringVar(value="Volume: --%")
        ...
        light_btn = ctk.CTkButton(row1, text="Light", command=lambda: self._set_system_light_mode(True),
                                  ...
                                  hover_color="#3A3A3C", text_color="white", width=48 if compact else 120)
        ...
        bri_minus_btn = ctk.CTkButton(brow, text="Bri-", command=lambda: (self._adjust_alpha(-0.05), self.after(80, self._refresh_settings_status)),
                                      corner_radius=14 if compact else 20, height=26 if compact else 32,
                                      font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                      hover_color="#3A3A3C", text_color="white", width=42 if compact else 120)
        ...
        vol_minus_btn = ctk.CTkButton(vrow, text="Vol-", command=lambda: (self._adjust_volume(-1), self.after(120, self._refresh_settings_status)),
                                      corner_radius=14 if compact else 20, height=26 if compact else 32,
                                      font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                      hover_color="#3A3A3C", text_color="white", width=42 if compact else 100)
    ...
    def _set_system_light_mode(self, enabled):
        ...
        try:
            ...
            pass
        self.toast.show("Light mode" if enabled else "Dark mode", 900)
