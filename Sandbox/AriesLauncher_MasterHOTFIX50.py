...
class VAApp(ctk.CTk):
    ...
    def show_browser(self, search_query=None):
        ...
        # Compact top toolbar only so Chromium remains visible below.
        self._vf.place(x=0, y=0, width=WIDTH, height=178)
        self._vf.tkraise()
        self._vf_visible = True
        ...
        top.pack(fill="x")
        top.pack_propagate(False)
        ctk.CTkLabel(top, text="â€¹  Browser", font=(FONT, 18), text_color=C).pack(side="left", padx=24, pady=12)
        ctk.CTkLabel(top, text="Long press exits", font=(FONT, 10), text_color="#48484A").pack(side="right", padx=24, pady=12)
        ctk.CTkFrame(self._vf, fg_color="#2C2C2E", height=1).pack(fill="x", padx=0)

        shell = ctk.CTkFrame(self._vf, corner_radius=12, fg_color=PNL)
        shell.pack(fill="both", expand=True, padx=14, pady=(8, 8))
        ...
        ctl = ctk.CTkFrame(shell, fg_color="transparent")
        ctl.pack(fill="x", padx=10, pady=(0, 10))
        self._btn(ctl, "Prev", self._browser_focus_prev, w=68)
        self._btn(ctl, "Next", self._browser_focus_next, w=68)
        self._btn(ctl, "Click", self._browser_activate, True, 72)
        self._btn(ctl, "PgUp", self._browser_scroll_up, w=68)
        self._btn(ctl, "PgDn", self._browser_scroll_down, w=68)

        help_row = ctk.CTkLabel(
            shell,
            text="Rotate = Prev/Next focus  ·  Click = activate  ·  Long press = exit  ·  Type = keyboard",
            font=(FONT, 11),
            text_color=TXTD,
            justify="center",
        )
        help_row.pack(pady=(0, 8))
    ...
    def show_settings(self):
        ...
        self.after(50, self._refresh_settings_status)

        row1 = ctk.CTkFrame(p, fg_color="transparent")
        row1.pack(fill="x", padx=20, pady=(14, 8))
        self._btn(row1, "SystemLight", lambda: self._set_system_light_mode(True), w=120)
        self._btn(row1, "SystemDark", lambda: self._set_system_light_mode(False), w=120)

        ctk.CTkLabel(p, textvariable=self._settings_brightness_var,
                      font=(FONT, 13, "bold"), text_color=TXT).pack(anchor="w", padx=20, pady=(12, 4))
        brow = ctk.CTkFrame(p, fg_color="transparent")
        brow.pack(fill="x", padx=20, pady=(0, 8))
        self._btn(brow, "Brightness-", lambda: (self._adjust_alpha(-0.05), self.after(50, self._refresh_settings_status)), w=130)
        self._btn(brow, "Brightness+", lambda: (self._adjust_alpha(+0.05), self.after(50, self._refresh_settings_status)), w=130)

        ctk.CTkLabel(p, textvariable=self._settings_volume_var,
                      font=(FONT, 13, "bold"), text_color=TXT).pack(anchor="w", padx=20, pady=(12, 4))
        vrow = ctk.CTkFrame(p, fg_color="transparent")
        vrow.pack(fill="x", padx=20, pady=(0, 8))
        self._btn(vrow, "Volume-", lambda: (self._adjust_volume(-1), self.after(50, self._refresh_settings_status)), w=110)
        self._btn(vrow, "Volume+", lambda: (self._adjust_volume(+1), self.after(50, self._refresh_settings_status)), w=110)
        ...
        if self.rotary.available:
            ...
            ctk.CTkLabel(p, text="Rotary Encoder â€” Connected",
                          font=(FONT, 12), text_color=GRN).pack(anchor="w", padx=20)
