...
class VAApp(ctk.CTk):
    ...
    def _touchpad_move(self, x, y):
        ...
        try:
            ...
            if not self._keyboard_drag_started:
                ...
                overshoot = self._keyboard_drag_pending - (self._KEYBOARD_DRAG_START_PX if self._keyboard_drag_pending > 0 else -self._KEYBOARD_DRAG_START_PX)
    ...
    _KEYBOARD_DRAG_START_PX = 240.0
    _KEYBOARD_STEP_PX = 260.0

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 260.0
    _HOME_DRAG_START_PX = 180.0
    _HOME_ANCHOR_RESET_SEC = 0.14
    _HOME_NOISE_FLOOR = 18
    _HOME_HOLD_FREEZE_SEC = 0.18
    _HOME_IDLE_FREEZE_SEC = 0.08
    ...
    def _home_touch_track(self, x, y):
        ...
        if not self._home_drag_started:
            ...
            overshoot = self._home_drag_pending - (self._HOME_DRAG_START_PX if self._home_drag_pending > 0 else -self._HOME_DRAG_START_PX)
    ...
    def _touchpad_tap(self):
        ...
        if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.20:
            return
        if (now - getattr(self, "_last_touchpad_motion_at", 0.0)) < 0.12:
            return
