...
class VAApp(ctk.CTk):
        ...
        try:
            ...
            for line in out.splitlines():
                ...
                if "max98357a" in low:
                    ...
                    if m:
                        self._speaker_hw_cache = f"hw:{m.group(1)},{m.group(2)}"
        ...
        try:
            ...
            for raw in out.splitlines():
                ...
                if m:
                    ...
                    if "stereo" in desc or "speaker" in desc or "analog" in desc:
                        self._speaker_sink_cache = sink_id
                        return sink_id
        except Exception:
            pass
    ...
    def _find_volume_target(self):
        preferred = []
        hw = self._speaker_hw_device()
        if hw.startswith("hw:"):
            card = hw.split(":", 1)[1].split(",", 1)[0]
            preferred.append(card)
        preferred.extend(["2", "0", "1", "3"])
        seen = set()
        preferred_cards = []
        ...
        preferred_names = ["Speaker", "PCM", "Master", "Headphone", "Playback", "Digital"]
        for card in preferred_cards:
            ...
            for name in preferred_names:
                if f"'{name}'" in out:
                    return (card, name)
        try:
            out = subprocess.check_output(["amixer", "scontrols"], stderr=subprocess.DEVNULL, text=True)
            for name in preferred_names:
                if f"'{name}'" in out:
                    return (None, name)
        except Exception:
    ...
    def _adjust_volume(self, delta):
        ...
        target = getattr(self, "_volume_target", None) or self._find_volume_target()
        if target:
            self._volume_target = target
            card, name = target
            try:
                cmd = ["amixer", "sset", name, step] if card is None else ["amixer", "-c", card, "sset", name, step]
                result = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
                if result.returncode == 0:
                    self._last_volume_mode = f"alsa {name}" if card is None else f"alsa card {card} {name}"
                    self.toast.show(f"Volume {self._query_volume_pct() or ''}%".strip())
                    self.status.append("Volume applied")
                    return
            except Exception:
    ...
    def _move_focus(self, delta):
        ...
        current = self._nav_focus_widget if self._widget_alive(self._nav_focus_widget) else (self._resolve_text_input_widget(self.focus_get()) or self.focus_get() or self._active_input)
        try:
    ...
    def _global_move(self, direction):
        ...
        if self.current_view == "browser" and self._browser_is_open():
            if direction in {"left", "up", "prev"}:
                self._browser_focus_prev()
                return
            elif direction in {"right", "down", "next"}:
                self._browser_focus_next()
                return
    ...
    def _invoke_focused_widget(self):
        widget = self._nav_focus_widget if self._widget_alive(self._nav_focus_widget) else self.focus_get()
        if widget is None:
            return False
    ...
    def _ensure_vf(self):
        if self._vf is None:
            self._vf = tk.Frame(self, bg=BG, highlightthickness=0, cursor="none")
    ...
    def _view(self, vid, title, body="", placeholder=False):
        ...
        top = ctk.CTkFrame(self._vf, fg_color="transparent", height=40 if compact else 52)
        top.pack(fill="x")
        top.pack_propagate(False)
        ctk.CTkLabel(top, text=f"‹  {title}", font=(FONT, 15 if compact else 18),
                      text_color=C).pack(side="left", padx=10 if compact else 24, pady=7 if compact else 12)
        ctk.CTkLabel(top, text="Esc back",
                      font=(FONT, 10 if compact else 10),
                      text_color="#48484A").pack(side="right", padx=10 if compact else 24, pady=7 if compact else 12)

        ctk.CTkFrame(self._vf, fg_color="#2C2C2E", height=1).pack(fill="x", padx=0)

        if body:
            ctk.CTkLabel(self._vf, text=body, font=(FONT, 10 if compact else 11),
                          text_color=TXTD, wraplength=WIDTH - (52 if compact else 100),
                          justify="center").pack(pady=(3 if compact else 8, 2))

        panel = ctk.CTkFrame(self._vf, corner_radius=8 if compact else 14, fg_color=PNL,
                              border_color=PNLE, border_width=0)
        panel.pack(pady=(2 if compact else 6, 5 if compact else 10),
                   padx=8 if compact else 24, fill="both", expand=True)

        if placeholder:
            ctk.CTkLabel(panel, text=f"{title} — coming soon",
                          font=(FONT, 12 if compact else 14), text_color=TXTD,
                          justify="center").place(relx=.5, rely=.5, anchor="center")
        return panel

    def _show_swipe_hint(self, direction):
        label = ""
        ...
        if not label:
            return
        try:
            self.cv.itemconfigure(self._swipe_hint_id, text=label)
            if self._swipe_hint_after:
                self.after_cancel(self._swipe_hint_after)
            self._swipe_hint_after = self.after(
                500, lambda: self.cv.itemconfigure(self._swipe_hint_id, text="")
            )
        except Exception:
            pass
    ...
    def _touchpad_swipe(self, direction):
        ...
        self._show_swipe_hint(direction)
    ...
    def _touchpad_move(self, x, y):
        x, y = self._transform_touch_axes(x, y)
        # ── Home screen: slow, deliberate horizontal drag only ────────────────
        if self.current_view == "home" and not self._touch_keyboard.visible:
        ...
        if not self._touch_keyboard.visible:
            self._keyboard_drag_last_x = None
            self._keyboard_drag_last_y = None
            self._keyboard_drag_accum = 0.0
            self._keyboard_drag_pending = 0.0
            self._keyboard_drag_started = False
            return

        # ── Keyboard: rigid horizontal selector, not a free cursor ───────────
        try:
            now = time.perf_counter()
            if now < getattr(self, "_keyboard_ignore_until", 0.0):
                self._keyboard_drag_last_x = x
                self._keyboard_drag_last_y = y
                self._keyboard_drag_accum = 0.0
                self._keyboard_drag_pending = 0.0
                self._keyboard_drag_started = False
                return
            ...
            dy = y - self._keyboard_drag_last_y
            self._keyboard_drag_last_x = x
            self._keyboard_drag_last_y = y

            # Ignore tiny noise and diagonal wobble. The keyboard should only
            # react to clearly horizontal movement.
            if abs(dx) <= self._KEYBOARD_NOISE_FLOOR:
                return
            if abs(dx) < max(self._KEYBOARD_NOISE_FLOOR * 1.5, abs(dy) * self._KEYBOARD_AXIS_RATIO):
                return
            ...
            while self._keyboard_drag_accum >= self._KEYBOARD_STEP_PX:
                ...
                self._show_swipe_hint("left")
                moved = True
            while self._keyboard_drag_accum <= -self._KEYBOARD_STEP_PX:
                self._keyboard_drag_accum += self._KEYBOARD_STEP_PX
                self._touch_keyboard.sel_col = min(len(self._touch_keyboard.TOKENS) - 1, self._touch_keyboard.sel_col + 1)
                self._show_swipe_hint("right")
                moved = True

            if moved:
                self._keyboard_last_move_at = now
                self._keyboard_tap_ignore_until = max(
                    getattr(self, "_keyboard_tap_ignore_until", 0.0),
                    now + self._KEYBOARD_POST_MOVE_TAP_BLOCK_SEC,
                )
    ...
    _KEYBOARD_SHOW_SETTLE_SEC = 0.28
    _KEYBOARD_TAP_SETTLE_SEC = 0.26
    _KEYBOARD_POST_MOVE_TAP_BLOCK_SEC = 0.24
    _KEYBOARD_NOISE_FLOOR = 24
    _KEYBOARD_AXIS_RATIO = 1.8
    _KEYBOARD_DRAG_START_PX = 520.0    # force a clearly intentional drag before the rail moves
    _KEYBOARD_STEP_PX = 1100.0         # one large, deliberate drag per letter

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 2600.0             # much slower app stepping; one normal swipe ≈ one app
    _HOME_DRAG_START_PX = 520.0        # require a clearly intentional swipe before moving apps
    _HOME_ANCHOR_RESET_SEC = 0.16
    _HOME_NOISE_FLOOR = 34
    _HOME_HOLD_FREEZE_SEC = 0.14
    _HOME_IDLE_FREEZE_SEC = 0.06
    _TOUCH_SWAP_AXES = False
    _TOUCH_INVERT_X = False
    _TOUCH_INVERT_Y = False

    def _transform_touch_axes(self, x, y):
        tx, ty = x, y
        if self._TOUCH_SWAP_AXES:
            tx, ty = ty, x
    ...
    def _home_touch_track(self, x, y):
        ...
        Simpler than the axis-lock build: only horizontal delta matters here,
        with a bigger deadzone and much slower step size so swipes feel deliberate.
        """
        now = time.perf_counter()
        ...
        # Ignore tiny jitter and mostly-vertical drift. Home should react only
        # to clearly horizontal swipes.
        if abs(dx) <= self._HOME_NOISE_FLOOR or abs(dx) < max(self._HOME_NOISE_FLOOR * 1.2, abs(dy) * 1.6):
    ...
    def _touchpad_tap(self):
        ...
        if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.34:
            return
        if (now - getattr(self, "_home_last_move_at", 0.0)) < 0.16:
            return
        if self._typing() and not self._touch_keyboard.visible:
            self._show_touch_keyboard()
            return
        if self._touch_keyboard.visible:
            if now < getattr(self, "_keyboard_tap_ignore_until", 0.0):
                return
            if (now - getattr(self, "_keyboard_last_move_at", 0.0)) < 0.22:
                return
        self._global_select()
    ...
    def _show_touch_keyboard(self):
        ...
        if self.current_view == "home" or not self._active_input_alive():
            ...
            return
        already_visible = self._touch_keyboard.visible
        self.log.debug("Showing touch keyboard for input: %s", self._active_input)
        self._touch_keyboard.show()
        if not already_visible:
            now = time.perf_counter()
            self._keyboard_ignore_until = now + self._KEYBOARD_SHOW_SETTLE_SEC
            self._keyboard_tap_ignore_until = now + self._KEYBOARD_TAP_SETTLE_SEC
            self._keyboard_last_move_at = 0.0
    ...
    def _show_photo_delete_keyboard(self):
        self._active_input = None
        self._active_submit = None
        self._touch_keyboard.ensure()
        self._touch_keyboard._symbol_mode = False
        self._touch_keyboard.sel_row = 0
        self._touch_keyboard.sel_col = 0
        self._touch_keyboard._viewport_start = 0
        self._touch_keyboard.show()
        self.toast.show("Delete menu", 700)

    def _photo_delete_current(self):
        current = getattr(self, "_pv_current", None)
        if current and os.path.exists(current):
            try:
                ...
                self.toast.show(f"Delete failed: {exc}")

    def _handle_touch_key(self, token):
        if self._browser_typing_mode and self.current_view == "browser" and self._browser_is_open():
            if token == TouchKeyboard.TOK_SPACE:
                self._browser_type_text(" ")
                return
            if token == TouchKeyboard.TOK_BACK:
                self._browser_send_key("BackSpace")
                return
            if token == TouchKeyboard.TOK_ENTER:
                self._browser_send_key("Return")
                self._browser_typing_mode = False
                self._hide_touch_keyboard()
                return
            self._browser_type_text(token)
            return
    ...
    def show_assistant(self):
        p = self._view("assistant", "Voice Mode",
                       "Tap once to activate Aries voice mode.")

        card = ctk.CTkFrame(p, fg_color="transparent")
        card.pack(expand=True)

        state_var = tk.StringVar(value=f"State: {self.voice.state.value}")
        ctk.CTkLabel(card, text="Aries Voice Mode",
                     font=(FONT, 18, "bold"), text_color=TXT).pack(pady=(16, 6))
        ctk.CTkLabel(card, textvariable=state_var,
                     font=(FONT, 12), text_color=TXTD).pack(pady=(0, 16))

        def start_voice():
            activated = self.voice.activate()
            if activated:
                state_var.set("State: LISTENING")
                self.toast.show("Aries voice mode")
            else:
                state_var.set(f"State: {self.voice.state.value}")
    ...
    def show_camera(self):
        ...
        self._cam_label = ctk.CTkLabel(
            p,
            text="Initializing …",
            font=(FONT, 12 if compact else 13),
            text_color=TXTD,
        )
        self._cam_label.pack(fill="both", expand=True)

        bb = ctk.CTkFrame(self._vf, fg_color="transparent")
        bb.pack(pady=(0, 4 if compact else 8))

        capture_btn = ctk.CTkButton(
            bb,
            text="Capture",
            command=self._capture_photo,
            corner_radius=14 if compact else 20,
            height=26 if compact else 32,
            font=(FONT, 9, "bold") if compact else (FONT, 12, "bold"),
            fg_color=C,
            hover_color=CD,
            text_color="white",
            width=84 if compact else 108,
        )
        capture_btn.pack(side="left", padx=2 if compact else 4)
        self._cam_capture_btn = capture_btn

        restart_btn = ctk.CTkButton(
            bb,
            text="Restart",
            command=self._restart_cam,
            corner_radius=14 if compact else 20,
            height=26 if compact else 32,
            font=(FONT, 9) if compact else (FONT, 12),
            fg_color=PNLE,
            hover_color="#3A3A3C",
            text_color="white",
            width=46 if compact else None,
        )
        restart_btn.pack(side="left", padx=2 if compact else 4)
        ...
        stop_btn = ctk.CTkButton(
            ...
            hover_color="#3A3A3C",
            text_color="white",
            width=38 if compact else None,
        )
        stop_btn.pack(side="left", padx=2 if compact else 4)

        self._prime_focus(capture_btn)
        self._start_camera()
    ...
    def _is_capture_device(self, device_path):
        try:
            ...
            blob = (result.stdout + "\n" + result.stderr).lower()
            return ("video capture" in blob) or ("capture" in blob and "not a video capture device" not in blob)
        except Exception:
            return True

    def _open_camera_capture(self, device_path):
        backends = []
        ...
        for backend in backends:
            cap = None
            try:
                cap = cv2.VideoCapture(device_path, backend) if backend is not None else cv2.VideoCapture(device_path)
    ...
    def _start_camera(self):
        ...
        for idx in self._camera_candidate_indices():
            try:
                ...
                if cap is not None:
                    ...
                    self.log.info("Camera opened on source %s", idx)
                    break
            except Exception as exc:
                last_error = exc
                self.log.exception("Camera initialization failed on source %s", idx)
        ...
        if not self._cam_cap or not self._cam_cap.isOpened():
            ...
            self.toast.show("Camera not detected")
            return
        self._cam_running = True
        self.toast.show(f"Camera live on /dev/video{self._cam_source_index}", 1200)
        self._cam_tick()
    ...
    def _cam_tick(self):
        ...
        try:
            ...
            if ret:
                ...
                self._cam_last_frame = full_img.copy()
                preview_size = (600, 295) if (WIDTH <= 640 or HEIGHT <= 400) else (1020, 600)
                img = full_img.resize(preview_size)
                self._cam_image = _make_ctk_image(img, preview_size)
                if self._cam_label:
    ...
    def _capture_photo(self):
        ...
        if self._cam_cap:
            try:
                ok, raw = self._cam_cap.read()
                if ok and raw is not None:
                    raw = cv2.cvtColor(raw, cv2.COLOR_BGR2RGB)
                    frame = Image.fromarray(raw)
                    self._cam_last_frame = frame.copy()
            except Exception:
                frame = None

        if frame is None and self._cam_last_frame is not None:
            try:
                frame = self._cam_last_frame.copy()
            except Exception:
                frame = self._cam_last_frame

        if frame is None:
            self.toast.show("No frame to capture")
            self.status.append("Capture failed: no frame")
            return
        ...
        try:
            ...
            if os.path.exists(path) and os.path.getsize(path) > 0:
                self.toast.show("Captured!", 1200)
                self.status.append(f"Captured {os.path.basename(path)}")
                if self._widget_alive(self._cam_capture_btn):
                    self._cam_capture_btn.configure(text="Captured ✓")
                    if self._cam_capture_reset_after:
                        try:
                            self.after_cancel(self._cam_capture_reset_after)
                        except Exception:
                            pass
                    self._cam_capture_reset_after = self.after(1400, self._reset_capture_button)
            else:
                self.toast.show("Save failed")
                self.status.append("Capture failed: file missing after save")
        except Exception as e:
            self.toast.show(f"Save failed: {e}")
            self.status.append(f"Capture failed: {e}")
    ...
    def show_photos(self):
        ...
        preview_w, preview_h = ((360, 100) if (WIDTH <= 640 or HEIGHT <= 400) else (520, 280))
        self._pv_label = ctk.CTkLabel(p, text="", fg_color="#1C1C1E",
                                       corner_radius=12, width=preview_w, height=preview_h)
        ...
        scroll = ctk.CTkScrollableFrame(p, fg_color="transparent",
                                         scrollbar_button_color="#48484A", height=(56 if (WIDTH <= 640 or HEIGHT <= 400) else 130))
        scroll.pack(fill="x", padx=8, pady=(0, 4))

        row = ctk.CTkFrame(scroll, fg_color="transparent")
        row.pack(fill="x")
        first_thumb_btn = None
        ...
        for i, fname in enumerate(files):
            ...
            try:
                thumb_size = (56, 32) if (WIDTH <= 640 or HEIGHT <= 400) else (130, 82)
                thumb_btn = (62, 38) if (WIDTH <= 640 or HEIGHT <= 400) else (138, 90)
                thumb = Image.open(fpath).convert("RGBA").resize(thumb_size, Image.LANCZOS)
                ctk_img = _make_ctk_image(thumb, thumb_size)
                self._pv_refs.append(ctk_img)
                thumb_button = ctk.CTkButton(row, image=ctk_img, text="", width=thumb_btn[0], height=thumb_btn[1],
                               corner_radius=8, fg_color=PNLE, hover_color="#3A3A3C",
                               command=lambda fp=fpath, fn=fname: self._pv_show(fp, fn)
                               )
                thumb_button.pack(side="left", padx=3)
                if first_thumb_btn is None:
                    first_thumb_btn = thumb_button
            except Exception:
                pass
        ...
        if first_thumb_btn is not None:
            self._prime_focus(first_thumb_btn)

    def _pv_show(self, path, name=""):
        self._pv_current = path
        try:
            img = Image.open(path).convert("RGBA")
            img.thumbnail((560, 210) if (WIDTH <= 640 or HEIGHT <= 400) else (520, 280), Image.LANCZOS)
    ...
    def _browser_go_back(self):
        if self._browser_send_key("Alt+Left"):
            self.toast.show("Browser: back", 700)

    def _browser_go_forward(self):
        if self._browser_send_key("Alt+Right"):
            self.toast.show("Browser: forward", 700)

    def _browser_shift_left(self):
        if self._browser_send_key("Left"):
            self.toast.show("Cursor left", 500)

    def _browser_shift_right(self):
        if self._browser_send_key("Right"):
            self.toast.show("Cursor right", 500)

    def _browser_clear_entry(self):
        cleared = False
        if self._browser_typing_mode and self.current_view == "browser" and self._browser_is_open():
            try:
                cleared = self._browser_send_key("ctrl+a", "BackSpace")
                if not cleared:
                    cleared = self._browser_send_key("ctrl+a", "Delete")
            except Exception:
                cleared = False
        if self._b_url is not None:
            self._b_url.set("")
            cleared = True
        if self._browser_entry is not None:
            try:
                self._browser_entry.delete(0, "end")
                self._browser_entry.focus_set()
                cleared = True
            except Exception:
                pass
        self._active_input = self._browser_entry
        self._active_submit = lambda: self._browser_launch(self._b_url.get())
        if cleared:
            self.toast.show("Cleared", 700)
        else:
            self.toast.show("Clear failed", 900)

    def _browser_window_ids(self):
        xdotool = shutil.which("xdotool")
        if not xdotool:
            return []
        for class_name in ("chromium", "Chromium", "chromium-browser", "Chromium-browser"):
            try:
    ...
    def _browser_can_remote_control(self):
        return bool(shutil.which("xdotool")) and bool(self._browser_window_ids())

    def _browser_send_key(self, *keys):
        xdotool = shutil.which("xdotool")
        ids = self._browser_window_ids()
        if not xdotool or not ids:
            self.toast.show("Install xdotool or reopen browser")
            self.status.append("Browser control unavailable")
            return False
        wid = ids[-1]
        try:
            subprocess.run([xdotool, "windowactivate", "--sync", wid],
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
            subprocess.run([xdotool, "key", "--window", wid, *keys],
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
            return True
        except Exception as exc:
            self.toast.show(f"Browser control failed: {exc}")
            return False

    def _browser_type_text(self, text):
        xdotool = shutil.which("xdotool")
        ids = self._browser_window_ids()
        if not xdotool or not ids:
            self.toast.show("Install xdotool or reopen browser")
            self.status.append("Browser typing unavailable")
            return False
        wid = ids[-1]
        try:
            subprocess.run([xdotool, "windowactivate", "--sync", wid],
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
            subprocess.run([xdotool, "type", "--delay", "0", "--window", wid, text],
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
            return True
        except Exception as exc:
            self.toast.show(f"Browser typing failed: {exc}")
            return False

    def _browser_focus_prev(self):
        if not shutil.which("xdotool"):
            self.toast.show("sudo apt install xdotool  (needed for browser scroll)", 2500)
            return
        if self._browser_send_key("shift+Tab"):
            self.toast.show("◀ prev element", 500)

    def _browser_focus_next(self):
        if not shutil.which("xdotool"):
            self.toast.show("sudo apt install xdotool  (needed for browser scroll)", 2500)
            return
        if self._browser_send_key("Tab"):
            self.toast.show("▶ next element", 500)

    def _browser_activate(self):
        if self._browser_send_key("Return"):
            self.toast.show("Browser: activate", 700)

    def _browser_scroll_up(self):
        if self._browser_send_key("Prior"):
            self.toast.show("Browser: page up", 700)

    def _browser_scroll_down(self):
        if self._browser_send_key("Next"):
            self.toast.show("Browser: page down", 700)


    def _browser_show_type_keyboard(self):
        if not self._browser_can_remote_control():
            self.toast.show("Install xdotool or reopen browser")
            return
        self._browser_typing_mode = True
        self._active_input = self._browser_entry
        self._active_submit = None
        self._touch_keyboard.ensure()
        self._touch_keyboard._symbol_mode = False
        self._touch_keyboard.sel_row = 0
        tokens = self._touch_keyboard._tokens()
        try:
            self._touch_keyboard.sel_col = tokens.index(TouchKeyboard.TOK_CAPS)
        except Exception:
            self._touch_keyboard.sel_col = 0
        self._touch_keyboard._viewport_start = 0
        self._touch_keyboard.show()
        self.toast.show("Keyboard open", 900)
    ...
    def show_browser(self, search_query=None):
        ...
        compact = WIDTH <= 640 or HEIGHT <= 400
        overlay_h = 78 if compact else 118
        top_h = 26 if compact else 38

        self._vf.place(x=0, y=0, width=WIDTH, height=overlay_h)
        self._vf.tkraise()
        self._vf_visible = True

        top = ctk.CTkFrame(self._vf, fg_color="transparent", height=top_h)
        top.pack(fill="x")
        top.pack_propagate(False)
        ctk.CTkLabel(
            top,
            text="‹ Browser",
            font=(FONT, 11 if compact else 16),
            text_color=C,
        ).pack(side="left", padx=8 if compact else 18, pady=4 if compact else 8)
        ctk.CTkLabel(
            top,
            text="2× click = keyboard   hold = exit",
            font=(FONT, 9 if compact else 10),
            text_color="#48484A",
        ).pack(side="right", padx=8 if compact else 18, pady=4 if compact else 8)
        ctk.CTkFrame(self._vf, fg_color="#2C2C2E", height=1).pack(fill="x", padx=0)

        shell = ctk.CTkFrame(self._vf, corner_radius=8 if compact else 12, fg_color=PNL)
        shell.pack(fill="both", expand=True, padx=5 if compact else 10, pady=(2 if compact else 6, 2 if compact else 6))

        nav = ctk.CTkFrame(shell, fg_color="transparent")
        nav.pack(fill="x", padx=4 if compact else 8, pady=(3 if compact else 8, 3 if compact else 8))
        ...
        ent = ctk.CTkEntry(
            ...
            font=(FONT, 11 if compact else 15),
            fg_color=PNL,
            text_color=TXT,
            border_color=C,
            height=28 if compact else 40,
        )
        ent.pack(side="left", fill="x", expand=True, padx=(0, 6))
        ...
        if compact:
            row1 = ctk.CTkFrame(shell, fg_color="transparent")
            row1.pack(fill="x", padx=4, pady=(0, 2))
            row2 = ctk.CTkFrame(shell, fg_color="transparent")
            row2.pack(fill="x", padx=4, pady=(0, 3))
            self._btn(row1, "Open", go, True, 58)
            self._btn(row1, "Home", self._browser_open_home, w=58)
            self._btn(row1, "Clear", self._browser_clear_entry, w=58)
            self._btn(row2, "Type", self._browser_show_type_keyboard, w=58)
            self._btn(row2, "Close", lambda: (self._close_browser(), self._go_home()), w=58)
        else:
            self._btn(nav, "Open", go, True, 72)
            self._btn(nav, "Home", self._browser_open_home, w=72)
            self._btn(nav, "Clear", self._browser_clear_entry, w=72)
            self._btn(nav, "Type", self._browser_show_type_keyboard, w=72)
        ...
        self.after(120, lambda: self._activate_text_input(ent, go))
        if search_query:
            self.after(180, go)
    ...
    def _open_external_browser(self, url):
        ...
        self.log.info("Browser request -> %s", url)
        ...
        if browser_cmd:
            ...
            try:
                ...
                self.log.exception("Chromium launch failed")
                self.toast.show(f"Chromium failed: {exc}")
        ...
        if not has_pywebview:
            ...
            ctk.CTkLabel(
                p,
                text="No browser available.\n\n"
                     "Install chromium/chromium-browser for the best experience.\n"
                     "pywebview/qt is fallback only.",
                font=(FONT, 13),
                text_color=TXTD,
                justify="center",
            ).place(relx=.5, rely=.5, anchor="center")
            self.toast.show("Install Chromium for browser support")
            self._focus_root()
            return
        ...
        script = (
            ...
            "frameless=False, easy_drag=False, resizable=False, confirm_close=False); "
    ...
    def show_translate(self):
        ...
        lv = ctk.StringVar(value=self._translate_target_var or "Japanese")
        self._translate_target_menu_var = lv
        self._translate_target_var = lv.get()
        self._translate_target_menu_var = lv
        self._translate_target_menu = ctk.CTkOptionMenu(top, values=langs, variable=lv, fg_color=PNL,
                           button_color=CD, button_hover_color=C,
                           text_color=TXT,
                           command=self._set_target_language)
        self._translate_target_menu.pack(side="left")

        src = ctk.CTkTextbox(p, height=(62 if (WIDTH <= 640 or HEIGHT <= 400) else 120), font=(FONT, 12 if (WIDTH <= 640 or HEIGHT <= 400) else 13),
                              fg_color="#1C1C1E", text_color=TXT)
        src.pack(fill="x", padx=10, pady=(8, 4))
        src.insert("end", "Hello, how are you?")
        src.focus_set()

        tgt = ctk.CTkTextbox(p, height=(62 if (WIDTH <= 640 or HEIGHT <= 400) else 120), font=(FONT, 12 if (WIDTH <= 640 or HEIGHT <= 400) else 13),
        ...
        def go():
            self._set_target_language(lv.get())
            user_text = src.get("1.0", "end").strip()
            res = self._translate(user_text, lv.get())
        ...
        self.after(120, lambda: self._activate_text_input(src, go))
    ...
    def show_settings(self):
        ...
        compact = WIDTH <= 640 or HEIGHT <= 400
        button_text = "black" if self._light_text else "white"
        self._settings_brightness_var = tk.StringVar(value=f"Brightness: {self._brightness_pct()}%")
        self._settings_volume_var = tk.StringVar(value=f"Volume: {self._query_volume_pct() or '--'}%")
        self.after(80, self._refresh_settings_status)

        padx = 8 if compact else 20
        row1 = ctk.CTkFrame(p, fg_color="transparent")
        row1.pack(fill="x", padx=padx, pady=(4, 2))
        light_btn = ctk.CTkButton(row1, text="Light", command=lambda: self._set_system_light_mode(True),
                                  corner_radius=14 if compact else 20, height=26 if compact else 32,
                                  font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                  hover_color="#3A3A3C", text_color=button_text, width=58 if compact else 120)
        light_btn.pack(side="left", padx=2 if compact else 4)
        dark_btn = ctk.CTkButton(row1, text="Dark", command=lambda: self._set_system_light_mode(False),
                                 corner_radius=14 if compact else 20, height=26 if compact else 32,
                                 font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                 hover_color="#3A3A3C", text_color=button_text, width=58 if compact else 120)
        dark_btn.pack(side="left", padx=2 if compact else 4)

        wifi_row = ctk.CTkFrame(p, fg_color="transparent")
        wifi_row.pack(fill="x", padx=padx, pady=(2, 4))
        wifi_btn = ctk.CTkButton(wifi_row, text="WiFi", command=self.show_wifi,
                                 corner_radius=14 if compact else 20, height=30 if compact else 38,
                                 font=(FONT, 10, "bold") if compact else (FONT, 13, "bold"), fg_color=PNLE,
                                 hover_color="#3A3A3C", text_color=button_text)
        wifi_btn.pack(fill="x", padx=2 if compact else 4)

        ctk.CTkLabel(p, textvariable=self._settings_brightness_var,
                     font=(FONT, 12 if compact else 13, "bold"), text_color=TXT).pack(anchor="w", padx=padx, pady=(3, 1))
        brow = ctk.CTkFrame(p, fg_color="transparent")
        brow.pack(fill="x", padx=padx, pady=(0, 2))
        bri_minus_btn = ctk.CTkButton(brow, text="Bri-", command=lambda: (self._adjust_alpha(-0.05), self.after(80, self._refresh_settings_status)),
                                      corner_radius=14 if compact else 20, height=26 if compact else 32,
                                      font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                      hover_color="#3A3A3C", text_color=button_text, width=52 if compact else 120)
        bri_minus_btn.pack(side="left", padx=2 if compact else 4)
        bri_plus_btn = ctk.CTkButton(brow, text="Bri+", command=lambda: (self._adjust_alpha(+0.05), self.after(80, self._refresh_settings_status)),
                                     corner_radius=14 if compact else 20, height=26 if compact else 32,
                                     font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                     hover_color="#3A3A3C", text_color=button_text, width=52 if compact else 120)
        bri_plus_btn.pack(side="left", padx=2 if compact else 4)

        ctk.CTkLabel(p, textvariable=self._settings_volume_var,
                     font=(FONT, 12 if compact else 13, "bold"), text_color=TXT).pack(anchor="w", padx=padx, pady=(3, 1))
        vrow = ctk.CTkFrame(p, fg_color="transparent")
        vrow.pack(fill="x", padx=padx, pady=(0, 2))
        vol_minus_btn = ctk.CTkButton(vrow, text="Vol-", command=lambda: (self._adjust_volume(-1), self.after(120, self._refresh_settings_status)),
                                      corner_radius=14 if compact else 20, height=26 if compact else 32,
                                      font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                      hover_color="#3A3A3C", text_color=button_text, width=52 if compact else 100)
        vol_minus_btn.pack(side="left", padx=2 if compact else 4)
        vol_plus_btn = ctk.CTkButton(vrow, text="Vol+", command=lambda: (self._adjust_volume(+1), self.after(120, self._refresh_settings_status)),
                                     corner_radius=14 if compact else 20, height=26 if compact else 32,
                                     font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                     hover_color="#3A3A3C", text_color=button_text, width=52 if compact else 100)
        vol_plus_btn.pack(side="left", padx=2 if compact else 4)
        speaker_test_btn = ctk.CTkButton(vrow, text="Test", command=self._run_speaker_test,
                                         corner_radius=14 if compact else 20, height=26 if compact else 32,
                                         font=(FONT, 9) if compact else (FONT, 12), fg_color=PNLE,
                                         hover_color="#3A3A3C", text_color=button_text, width=52 if compact else 100)
        speaker_test_btn.pack(side="left", padx=2 if compact else 4)

        ctk.CTkFrame(p, fg_color="#2C2C2E", height=1).pack(fill="x", padx=padx, pady=(4, 2))
        mics = VoiceController.list_microphones()
        mic_state = f"{len(mics)} device(s) detected" if mics else "No devices detected"
        ctk.CTkLabel(p, text=f"Microphone — {mic_state}",
                     font=(FONT, 12 if compact else 14, "bold"), text_color=TXT).pack(anchor="w", padx=padx)
        if mics:
            active_name = None
            for i, name in mics:
                if i == self.voice.mic_index or (self.voice.mic_index is None and active_name is None):
                    active_name = name
                    break
            if active_name:
                ctk.CTkLabel(p, text=f"Active: {active_name}", font=(FONT, 10 if compact else 11), text_color=TXTD, wraplength=WIDTH-(padx*2)).pack(anchor="w", padx=padx, pady=(2, 0))

        if self.rotary.available:
            ctk.CTkFrame(p, fg_color="#2C2C2E", height=1).pack(fill="x", padx=padx, pady=(2, 1))
            ctk.CTkLabel(p, text="Rotary Encoder — Connected", font=(FONT, 11 if compact else 12), text_color=GRN).pack(anchor="w", padx=padx)

        self._prime_focus(light_btn)
    ...
    def _apply_theme_palette(self):
        primary = "black" if self._light_text else "white"
        secondary = "#3C3C43" if self._light_text else TXTD
        try:
            self.cflow.text_color = primary
            self.cv.itemconfigure(self.cflow._label, fill=primary)
            self.cv.itemconfigure(self.status._time_id, fill=primary)
            self.cv.itemconfigure(self.status._sys_id, fill=secondary)
            self.cv.itemconfigure(self.status._msg_id, fill=secondary)
            self.cv.itemconfigure(self._mic_id, fill=secondary)
            self.cv.itemconfigure(self._timer_id, fill=primary)
            self.cv.itemconfigure(self._swipe_hint_id, fill=secondary)
            if getattr(self, "_hint_id", None):
                self.cv.itemconfigure(self._hint_id, fill=secondary)
        except Exception:
            pass

    def _set_system_light_mode(self, enabled):
        self.dark_mode = not enabled
        self._light_text = bool(enabled)
        ctk.set_appearance_mode("light" if enabled else "dark")
        try:
            self.configure(bg="#F2F2F7" if enabled else BG)
            self.cv.configure(bg="#F2F2F7" if enabled else BG)
        except Exception:
            pass
        self._apply_theme_palette()
        self.toast.show("Theme applied", 900)
        self.status.append("Appearance -> " + ("light" if enabled else "dark"))

    def _set_target_language(self, value):
        self._translate_target_var = value
        try:
            if getattr(self, "_translate_target_menu_var", None) is not None:
                self._translate_target_menu_var.set(value)
        except Exception:
            pass
        self.toast.show(f"Translate target -> {value}", 900)
        self.status.append(f"Translate target -> {value}")

    def _translate_target_button(self):
        if self.current_view != "translate":
            return
        langs = ["Japanese", "Spanish", "French", "Korean", "German", "Chinese"]
        current = self._translate_target_var if self._translate_target_var in langs else "Japanese"
        idx = langs.index(current)
        nxt = langs[(idx + 1) % len(langs)]
        self._set_target_language(nxt)
    ...
    def _query_volume_pct(self):
        ...
        if target:
            ...
            card, name = target
            try:
                cmd = ["amixer", "sget", name] if card is None else ["amixer", "-c", card, "sget", name]
                out = subprocess.check_output(cmd, stderr=subprocess.DEVNULL, text=True)
