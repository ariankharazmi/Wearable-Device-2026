    ...
    def _touchpad_move(self, x, y):
        ...
        try:
            ...
            if not self._keyboard_drag_started:
                ...
                self._keyboard_drag_accum = 0.0
                self._keyboard_drag_pending = 0.0
                self._keyboard_tap_ignore_until = now + 0.28
                return
            else:
            ...
            if moved:
                self._last_touchpad_swipe_at = time.perf_counter()
                self._keyboard_tap_ignore_until = now + 0.22
                self._touch_keyboard.refresh()
        except Exception:
            pass

    # Deliberate keyboard/home drag tuning for Cirque touchpad
    _KEYBOARD_SHOW_SETTLE_SEC = 0.26
    _KEYBOARD_TAP_SETTLE_SEC = 0.22
    _KEYBOARD_NOISE_FLOOR = 24
    _KEYBOARD_DRAG_START_PX = 420.0    # much stronger startup deadzone so taps do not become rail drags
    _KEYBOARD_STEP_PX = 1000.0         # far more rigid letter stepping; tiny motion should not jump letters

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 2200.0             # even slower app stepping to reduce accidental jumps
    _HOME_DRAG_START_PX = 460.0        # require a very intentional swipe before moving apps
    _HOME_ANCHOR_RESET_SEC = 0.14
    _HOME_NOISE_FLOOR = 26
    _HOME_HOLD_FREEZE_SEC = 0.12
    _HOME_IDLE_FREEZE_SEC = 0.05
    ...
    def _touchpad_tap(self):
        ...
        if self._touch_keyboard.visible:
            if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.34:
                return
            if (now - getattr(self, "_last_touchpad_motion_at", 0.0)) < 0.10:
                return
        else:
            if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.28:
                return
            if (now - getattr(self, "_last_touchpad_motion_at", 0.0)) < 0.08:
                return
        if self._typing() and not self._touch_keyboard.visible:
            self._show_touch_keyboard()
            return
        if self._touch_keyboard.visible and now < getattr(self, "_keyboard_tap_ignore_until", 0.0):
            return
        self._global_select()
