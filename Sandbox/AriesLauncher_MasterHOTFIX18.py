...
class VAApp(ctk.CTk):
    ...
    # Home-screen drag tuned for a Cirque pad behaving like a remote strip.
    _HOME_STEP_PX = 120
    _HOME_ANCHOR_RESET_SEC = 0.14
    _HOME_NOISE_FLOOR = 12
    _HOME_HOLD_FREEZE_SEC = 0.28
    _HOME_IDLE_FREEZE_SEC = 0.09

    def _home_touch_track(self, x, y):
        """Track relative finger travel like an Apple TV remote strip.

        Short, deliberate horizontal travel moves one app at a time.
        Long stationary holds do not keep generating movement from sensor noise.
        """
        now = time.perf_counter()
        ...
        # Once the finger has been held in place for a moment, freeze movement for
        # the rest of that touch so contact noise does not scroll through apps.
