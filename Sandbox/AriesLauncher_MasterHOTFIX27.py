...
class VAApp(ctk.CTk):
    ...
    def _touchpad_move(self, x, y):
        ...
        try:
            ...
            if self._keyboard_drag_last_x is None or self._keyboard_drag_last_y is None:
                self._keyboard_drag_last_x = x
                self._keyboard_drag_last_y = y
                self._keyboard_drag_accum = 0.0
                self._keyboard_drag_pending = 0.0
            ...
            if abs(dx) <= self._KEYBOARD_NOISE_FLOOR and abs(dy) <= self._KEYBOARD_NOISE_FLOOR:
                return

            if self._keyboard_axis_lock is None:
                if abs(dx) >= abs(dy) + self._KEYBOARD_AXIS_LOCK_MARGIN:
                    self._keyboard_axis_lock = "x"
                elif abs(dy) >= abs(dx) + self._KEYBOARD_AXIS_LOCK_MARGIN:
    ...
    def _home_touch_track(self, x, y):
        ...
        if (self._home_drag_last_x is None or self._home_drag_last_y is None
        ...
        if abs(dx) <= self._HOME_NOISE_FLOOR and abs(dy) <= self._HOME_NOISE_FLOOR:
        ...
        if self._home_axis_lock is None:
            if abs(dx) >= abs(dy) + self._HOME_AXIS_LOCK_MARGIN:
                self._home_axis_lock = "x"
            elif abs(dy) >= abs(dx) + self._HOME_AXIS_LOCK_MARGIN:
