...
class TouchKeyboard:
    """Compact iPad-style keyboard docked to the lower third of the display."""
    ...
    ROWS = [
        ...
        [("a", 1.0), ("s", 1.0), ("d", 1.0), ("f", 1.0), ("g", 1.0), ("h", 1.0), ("j", 1.0), ("k", 1.0), ("l", 1.0), (TOK_BACK, 1.55)],
        [("z", 1.0), ("x", 1.0), ("c", 1.0), ("v", 1.0), ("b", 1.0), ("n", 1.0), ("m", 1.0), (TOK_ENTER, 1.95)],
        [(TOK_SPACE, 5.9)],
    ]
    ROW_OFFSETS = [0.0, 0.42, 1.08, 2.28]
    TOTAL_UNITS = 10.95
    ...
    def ensure(self):
        ...
        hdr.pack(fill="x", padx=8, pady=(2, 1))
        ctk.CTkLabel(
            hdr,
            text="Keyboard",
            font=(FONT, 11, "bold"),
        ...
        preview = ctk.CTkFrame(shell, corner_radius=8, fg_color="#101013")
        preview.pack(fill="x", padx=8, pady=(0, 2))
        ctk.CTkLabel(
            preview,
        ...
        self._keys = ctk.CTkFrame(shell, fg_color="transparent")
        self._keys.pack(fill="both", expand=True, padx=8, pady=(0, 6))
        ...
        for r, row in enumerate(self.ROWS):
            row_buttons = []
            for c, (token, width_units) in enumerate(row):
                btn = ctk.CTkButton(
                    self._keys,
                ...
                self._token_to_pos[token] = (r, c)
                self._button_specs.append({"token": token, "button": btn, "row": r, "col": c, "units": width_units})
            self._row_buttons.append(row_buttons)
    ...
    def _position(self, force=False):
        ...
        abs_h = max(128, min(152, int(app_h * 0.34)))
        ...
        self._layout_keys(app_w, abs_h)
        self._last_geometry = geometry
        self.window.lift()
        self.window.tkraise()

    def _layout_keys(self, width, height):
        if self.window is None:
            return
        self.window.update_idletasks()

        usable_w = max(220, width - 16)
        key_area_h = max(78, height - 38)
        row_gap = 4
        total_row_gap = row_gap * 3
        row_h = max(22, min(30, int((key_area_h - total_row_gap) / 4)))

        top_y = 34
        side_pad = 8
        gap = 4
        unit_w = (usable_w - (2 * side_pad) - (gap * 10)) / self.TOTAL_UNITS

        for spec in self._button_specs:
            row = spec["row"]
            token = spec["token"]
            _, width_units = self.ROWS[row][spec["col"]]
            x_units = self.ROW_OFFSETS[row] + sum(w for _, w in self.ROWS[row][:spec["col"]])
            x = side_pad + (x_units * unit_w) + (spec["col"] * gap)
            y = top_y + row * (row_h + row_gap)
            w = max(22, int(width_units * unit_w + max(0, width_units - 1.0) * gap))
            spec["button"].place(x=int(x), y=int(y), width=w, height=row_h)
    ...
    def refresh(self):
        ...
        for spec in self._button_specs:
            token = spec["token"]
            selected = (spec["row"] == self.sel_row and spec["col"] == self.sel_col)
            flashing = token == self._flash_token
            spec["button"].configure(
                text=self._label(token),
    ...
    def move(self, dx=0, dy=0):
        ...
        if dy:
            row = max(0, min(len(self.ROWS) - 1, row + (1 if dy > 0 else -1)))
            col = min(col, len(self.ROWS[row]) - 1)

        if dx:
            col = max(0, min(len(self.ROWS[row]) - 1, col + (1 if dx > 0 else -1)))
    ...
    def activate(self):
        if not self.visible:
            return
        token = self.ROWS[self.sel_row][self.sel_col][0]
        self.press(token)
    ...
    def flash(self, token, persist_ms=120):
        if not self.visible:
            return
        pos = self._token_to_pos.get(token)
        if pos is not None:
    ...
    def track_touch(self, raw_x, raw_y):
        if not self.visible or self.window is None:
            return

        xn = (float(raw_x) - self.TOUCH_X_MIN) / max(1.0, (self.TOUCH_X_MAX - self.TOUCH_X_MIN))
        yn = (float(raw_y) - self.TOUCH_Y_MIN) / max(1.0, (self.TOUCH_Y_MAX - self.TOUCH_Y_MIN))
        xn = max(0.0, min(1.0, xn))
        yn = max(0.0, min(1.0, yn))

        self.window.update_idletasks()
        local_x = xn * max(1, self.window.winfo_width())
        local_y = yn * max(1, self.window.winfo_height())
        ...
        for spec in self._button_specs:
            ...
            if bx <= local_x <= bx + bw and by <= local_y <= by + bh:
                if (self.sel_row, self.sel_col) != (spec["row"], spec["col"]):
                    self.sel_row, self.sel_col = spec["row"], spec["col"]
                    self.refresh()
                return
...
class VAApp(ctk.CTk):
    ...
    def _touchpad_move(self, raw_x, raw_y):
        if self._touch_keyboard.visible:
            self._touch_keyboard.track_touch(raw_x, raw_y)
