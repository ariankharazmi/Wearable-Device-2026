...
class TouchpadController:
    ...
    def _finish_touch_if_needed(self, now):
        ...
        if not self._swiped_this_touch and (now - self._touch_started) <= self.TAP_WINDOW_SEC:
            if self._on_tap:
                self._on_tap()
...
class TouchKeyboard:
    """Compact docked QWERTY keyboard for 640x400, with direct touch tracking."""
    ...
    ROWS = [
        {"tokens": ["q", "w", "e", "r", "t", "y", "u", "i", "o", "p"], "offset": 0.03},
        {"tokens": ["a", "s", "d", "f", "g", "h", "j", "k", "l", TOK_BACK], "offset": 0.08},
        {"tokens": ["z", "x", "c", "v", "b", "n", "m", TOK_ENTER], "offset": 0.18},
        {"tokens": [TOK_SPACE], "offset": 0.23},
    ]
    ...
    def ensure(self):
        ...
        hdr = ctk.CTkFrame(shell, fg_color="transparent", height=18)
        hdr.grid(row=0, column=0, sticky="ew", padx=8, pady=(2, 1))
        ...
        preview = ctk.CTkFrame(shell, corner_radius=8, fg_color="#101013", height=26)
        preview.grid(row=1, column=0, sticky="ew", padx=8, pady=(0, 2))
        ...
        ctk.CTkLabel(
            ...
            justify="left",
            anchor="w",
        ).grid(row=0, column=0, sticky="ew", padx=8, pady=3)

        keys = ctk.CTkFrame(shell, fg_color="transparent")
        keys.grid(row=2, column=0, sticky="nsew", padx=6, pady=(0, 4))
        ...
        for r, row in enumerate(self.ROWS):
            row_buttons = []
            tokens = row["tokens"]
            offset_cols = int(round(row["offset"] * 10))
            if len(tokens) == 1:
                token = tokens[0]
                btn = self._make_button(keys, token)
                btn.grid(row=r, column=2, columnspan=6, padx=2, pady=2, sticky="nsew")
                row_buttons.append(btn)
            else:
                col = offset_cols
                remaining = max(0, 10 - offset_cols)
                base = remaining / max(len(tokens), 1)
                col_positions = []
                running = float(offset_cols)
                for i, token in enumerate(tokens):
                    start_col = int(round(running))
                    running += base
                    end_col = int(round(running)) if i < len(tokens) - 1 else 10
                    span = max(1, end_col - start_col)
                    if start_col + span > 10:
                        span = max(1, 10 - start_col)
                    col_positions.append((token, start_col, span))
                if col_positions:
                    last_token, last_col, last_span = col_positions[-1]
                    if last_col + last_span < 10:
                        col_positions[-1] = (last_token, last_col, 10 - last_col)
                for token, start_col, span in col_positions:
                    btn = self._make_button(keys, token)
                    btn.grid(row=r, column=start_col, columnspan=span, padx=2, pady=2, sticky="nsew")
                    row_buttons.append(btn)
            self._buttons.append(row_buttons)

    def _make_button(self, parent, token):
        return ctk.CTkButton(
            ...
            border_width=1,
            border_color="#2A2A2E",
            font=(FONT, 9, "bold") if token.startswith("__") else (FONT, 10),
            fg_color="#0A0A0B",
            hover_color="#171719",
            text_color=TXT,
        )
    ...
    def _position(self, force=False):
        ...
        abs_h = 138 if app_h <= 430 else 150
    ...
    def move(self, dx=0, dy=0):
        if not self.visible:
            return
        if dy:
            self.sel_row = max(0, min(len(self.ROWS) - 1, self.sel_row + (1 if dy > 0 else -1)))
            self.sel_col = min(self.sel_col, len(self.ROWS[self.sel_row]["tokens"]) - 1)
        if dx:
            row_len = len(self.ROWS[self.sel_row]["tokens"])
            self.sel_col = max(0, min(row_len - 1, self.sel_col + (1 if dx > 0 else -1)))
        self.refresh()

    def touch_move(self, raw_x, raw_y):
        if not self.visible:
            return
        try:
            nx = (float(raw_x) - self.PAD_X_MIN) / max(1.0, (self.PAD_X_MAX - self.PAD_X_MIN))
            ny = (float(raw_y) - self.PAD_Y_MIN) / max(1.0, (self.PAD_Y_MAX - self.PAD_Y_MIN))
        except Exception:
            return
        ...
        row_idx = min(len(self.ROWS) - 1, max(0, int(ny * len(self.ROWS))))
        ...
        if len(tokens) == 1:
            ...
            if right <= left:
                left, right = 0.0, 1.0
            local = (nx - left) / max(0.001, (right - left))
            local = min(1.0, max(0.0, local))
            col_idx = min(len(tokens) - 1, max(0, int(local * len(tokens))))

        if row_idx != self.sel_row or col_idx != self.sel_col:
    ...
    def activate(self):
        if not self.visible:
            return
        self.press(self.ROWS[self.sel_row]["tokens"][self.sel_col])

    def press(self, token):
        self.flash(token, persist_ms=120)
        self.app._handle_touch_key(token)
        self.refresh()

    def flash(self, token, persist_ms=120):
        if not self.visible:
            return
    ...
    def _find_token(self, token):
        for r, row in enumerate(self.ROWS):
            for c, candidate in enumerate(row["tokens"]):
                if candidate == token:
                    return (r, c)
        return None
...
class VAApp(ctk.CTk):
    ...
    def _touchpad_move(self, x, y):
        if self._touch_keyboard.visible:
            self._touch_keyboard.touch_move(x, y)
