...
class TouchpadController:
    ...
    def _handle_touch_point(self, x, y, now):
        ...
        total_dx = x - (self._touch_start_x if self._touch_start_x is not None else x)
        total_dy = y - (self._touch_start_y if self._touch_start_y is not None else y)
...
class TouchKeyboard:
    """Apple TV-style one-line keyboard with a moving viewport."""
    ...
    LAYOUT = [[
        TOK_SPACE, "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l",
    ...
    def _position(self, force=False):
        ...
        abs_h = max(126, min(156, int(app_h * 0.26)))
    ...
    def refresh(self):
        ...
        for slot, btn in enumerate(self._buttons):
            ...
            btn.configure(
                ...
                font=(FONT, 13, "bold") if token.startswith("__") else (FONT, 14),
            )
