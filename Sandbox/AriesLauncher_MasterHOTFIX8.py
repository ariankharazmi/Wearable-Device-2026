...
class TouchKeyboard:
    """Apple TV-style one-line keyboard for swipe-select + tap confirm."""
    ...
    def __init__(self, app):
        ...
        self._slot_tokens = [None] * self.VISIBLE_SLOTS
    ...
    def ensure(self):
        ...
        rail.grid(row=2, column=0, sticky="ew", padx=8, pady=(0, 8))
        for col in range(self.VISIBLE_SLOTS):
            rail.grid_columnconfigure(col, weight=1, uniform="appletv")

        self._buttons = []
        for idx in range(self.VISIBLE_SLOTS):
            btn = ctk.CTkButton(
                rail,
                text="",
                command=lambda i=idx: self._press_slot(i),
                height=44,
                corner_radius=10,
                border_width=1,
                border_color="#2A2A2E",
    ...
    def _position(self, force=False):
        ...
        abs_h = max(118, min(150, int(app_h * 0.23)))
    ...
    def _current_token(self):
        return self.LAYOUT[self.sel_row][self.sel_col]

    def _visible_window(self):
        tokens = self.LAYOUT[0]
        count = len(tokens)
        slots = self.VISIBLE_SLOTS
        sel = self.sel_col
        start = max(0, sel - slots // 2)
        if start + slots > count:
            start = max(0, count - slots)
        end = min(count, start + slots)
        window_tokens = tokens[start:end]
        if len(window_tokens) < slots:
            window_tokens += [None] * (slots - len(window_tokens))
        return start, window_tokens
    ...
    def refresh(self):
        ...
        start, visible_tokens = self._visible_window()
        self._slot_tokens = visible_tokens
        selected_index = self.sel_col - start

        for i, btn in enumerate(self._buttons):
            token = visible_tokens[i]
            if token is None:
                btn.configure(
                    text="",
                    state="disabled",
            ...
            wide = token in (self.TOK_SPACE, self.TOK_ENTER)
            btn.configure(
                text=self._label(token),
                state="normal",
                width=72 if not wide else 96,
                ...
                font=(FONT, 16, "bold") if selected else (FONT, 16),
            )
    ...
    def move(self, dx=0, dy=0):
        ...
        self.sel_col = max(0, min(len(self.LAYOUT[0]) - 1, self.sel_col + delta))
        self.refresh()

    def activate(self):
        if not self.visible:
            return
        self.press(self._current_token())

    def _press_slot(self, idx):
        if not self.visible:
            return
        token = self._slot_tokens[idx] if 0 <= idx < len(self._slot_tokens) else None
        if token is None:
            return
