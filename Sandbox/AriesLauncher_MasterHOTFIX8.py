...
class TouchpadController:
    ...
    MOVE_THRESHOLD = 10
    TAP_MAX_TRAVEL = 8
    SWIPE_DEBOUNCE_SEC = 0.10
    RELEASE_TIMEOUT_SEC = 0.10
    ...
    def _begin_touch(self, x, y, now):
        ...
        self._touch_start_x, self._touch_start_y = x, y
    ...
    def _handle_touch_point(self, x, y, now):
        ...
        total_dx = x - (self._touch_start_x or x)
        total_dy = y - (self._touch_start_y or y)
        self._max_travel = max(self._max_travel, abs(total_dx), abs(total_dy))

        if self._swiped_this_touch:
            return

        if max(abs(total_dx), abs(total_dy)) < self.MOVE_THRESHOLD:
            return
...
class TouchKeyboard:
    """Compact Apple TV-style one-line keyboard with a shifting viewport."""
    ...
    def __init__(self, app):
        ...
        self._linear_tokens = list(self.LAYOUT[0])
    ...
    def ensure(self):
        ...
        shell = ctk.CTkFrame(self.window, fg_color="#050506", corner_radius=14)
        shell.pack(fill="both", expand=True)
        shell.grid_columnconfigure(0, weight=1)

        self._preview_var = tk.StringVar(value="")
        preview = ctk.CTkFrame(shell, corner_radius=10, fg_color="#101013", height=28)
        preview.grid(row=0, column=0, sticky="ew", padx=10, pady=(8, 6))
        ...
        rail.grid(row=1, column=0, sticky="ew", padx=10, pady=(0, 8))
        for col in range(self.VISIBLE_SLOTS):
            rail.grid_columnconfigure(col, weight=1, uniform="atv")
        ...
        for col in range(self.VISIBLE_SLOTS):
            btn = ctk.CTkButton(
                rail,
                text="",
                command=lambda c=col: self._press_slot(c),
                height=44,
                corner_radius=10,
                border_width=1,
                border_color="#2A2A2E",
    ...
    def _selected_index(self):
        return max(0, min(len(self._linear_tokens) - 1, self.sel_col))

    def _sync_viewport(self):
        idx = self._selected_index()
        if len(self._linear_tokens) <= self.VISIBLE_SLOTS:
            self._viewport_start = 0
            return
        center = self.VISIBLE_SLOTS // 2
        start = idx - center
        start = max(0, min(start, len(self._linear_tokens) - self.VISIBLE_SLOTS))
    ...
    def _position(self, force=False):
        ...
        kb_w = min(app_w - 20, 620)
        kb_h = 106
        x = (app_w - kb_w) // 2
        y = app_h - kb_h - 8
        geometry = (x, y, kb_w, kb_h)
        if not force and geometry == self._last_geometry:
            return
        self.window.place(x=x, y=y, width=kb_w, height=kb_h)
    ...
    def refresh(self):
        ...
        for slot, btn in enumerate(self._buttons):
            idx = self._viewport_start + slot
            if idx >= len(self._linear_tokens):
                btn.configure(
                    text="", state="disabled",
                    fg_color="#050506", hover_color="#050506",
                    border_width=0, border_color="#050506", text_color="#050506"
                )
                continue

            token = self._linear_tokens[idx]
    ...
    def move(self, dx=0, dy=0):
        if not self.visible or not self._linear_tokens:
            return
        delta = 0
        if dx < 0:
            delta = -1
        ...
        self.sel_col = max(0, min(len(self._linear_tokens) - 1, self.sel_col + delta))
        self.refresh()
    ...
    def _press_slot(self, slot):
        idx = self._viewport_start + slot
        if 0 <= idx < len(self._linear_tokens):
            self.sel_col = idx
            self.press(self._linear_tokens[idx])

    def press(self, token):
        self.flash(token, persist_ms=160)
        self.app._handle_touch_key(token)
        self.refresh()

    def flash(self, token, persist_ms=160):
        if not self.visible:
            return
...
class VAApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self._last_touchpad_swipe_ui_at = 0.0
    ...
    def _touchpad_swipe(self, direction):
        self._last_touchpad_swipe_ui_at = time.perf_counter()
        if self._touch_keyboard.visible:
            if direction in {"left", "right"}:
                self._touch_keyboard.move(dx=-1 if direction == "left" else 1, dy=0)
            return
        self._global_move(direction)

    def _touchpad_tap(self):
        if (time.perf_counter() - getattr(self, "_last_touchpad_swipe_ui_at", 0.0)) < 0.22:
            return
