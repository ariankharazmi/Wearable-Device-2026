...
class VAApp(ctk.CTk):
    ...
    def _touchpad_swipe(self, direction):
        # Home screen navigation is handled only by the deliberate drag tracker.
        # Ignore low-level swipe callbacks here so hint text and tap suppression
        # do not fire when the home screen itself never actually moved.
    ...
    def _touchpad_move(self, x, y):
        ...
        try:
            ...
            if not self._keyboard_drag_started:
                ...
                overshoot = self._keyboard_drag_pending - (
                    self._KEYBOARD_DRAG_START_PX if self._keyboard_drag_pending > 0 else -self._KEYBOARD_DRAG_START_PX
                )
                self._keyboard_drag_accum = overshoot
                self._keyboard_drag_pending = 0.0
            else:
    ...
    _KEYBOARD_SHOW_SETTLE_SEC = 0.18
    _KEYBOARD_TAP_SETTLE_SEC = 0.16
    _KEYBOARD_POST_MOVE_TAP_BLOCK_SEC = 0.18
    _KEYBOARD_NOISE_FLOOR = 18
    _KEYBOARD_AXIS_RATIO = 1.5
    _KEYBOARD_DRAG_START_PX = 220.0    # force a clearly intentional drag before the rail moves
    _KEYBOARD_STEP_PX = 420.0         # one large, deliberate drag per letter

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 900.0             # much slower app stepping; one normal swipe ≈ one app
    _HOME_DRAG_START_PX = 180.0        # require a clearly intentional swipe before moving apps
    _HOME_ANCHOR_RESET_SEC = 0.14
    _HOME_NOISE_FLOOR = 20
    _HOME_HOLD_FREEZE_SEC = 0.18
    ...
    def _home_touch_track(self, x, y):
        ...
        if not self._home_drag_started:
            ...
            overshoot = self._home_drag_pending - (
                self._HOME_DRAG_START_PX if self._home_drag_pending > 0 else -self._HOME_DRAG_START_PX
            )
            self._home_move_residual = overshoot
            self._home_drag_pending = 0.0
        else:
