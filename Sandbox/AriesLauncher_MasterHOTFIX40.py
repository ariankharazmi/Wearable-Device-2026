...
class VAApp(ctk.CTk):
    ...
    def _open_external_browser(self, url):
        ...
        # Prefer a real browser first. Under bare xinit with no window manager,
        # Chromium app mode is much more dependable than spawning another
        # unmanaged pywebview top-level window.
        browser_cmd = None
        ...
        # Fallback to pywebview if Chromium is unavailable.
        try:
        ...
        if not has_pywebview:
            ...
            ctk.CTkLabel(p, text="No browser available.\n\n"
                          "Install chromium or pywebview/qt.\n"
                          "tkinterweb is basic fallback only.",
                          font=(FONT, 13), text_color=TXTD,
                          justify="center").place(relx=.5, rely=.5, anchor="center")
            self._focus_root()
            return
