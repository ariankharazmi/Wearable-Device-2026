...
class TouchpadController:
    ...
    RELEASE_TIMEOUT_SEC = 0.012
    JITTER_FLOOR = 1
    JUMP_THRESHOLD = 1800
    SWIPE_DISTANCE = 28
    SWIPE_RESET_RATIO = 0.50
...
class VAApp(ctk.CTk):
    ...
    def _touchpad_move(self, x, y):
        ...
        # Map the raw Cirque X coordinate to the visible keyboard rail so the highlight
        # tracks the finger position instead of feeling like a stuck rotary selector.
        try:
            span = max(1, len(self._touch_keyboard.TOKENS) - 1)
            normalized = max(0.0, min(1.0, x / 4095.0))
            target = int(round(normalized * span))
    ...
    def _touchpad_tap(self):
        if (time.perf_counter() - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.08:
            return
