...
if IS_PI:
    ...
    TARGET_FPS = 30
    BASE_ICON = 76          # bigger icons for 0.23" display
    SPACING = 96            # comfortable spacing, all 5 icons fit in 640px
    FLOW_LAMBDA = 18.0
    SIZE_STEP_PX = 4
    ALPHA_STEP_8 = 32
else:
...
class VAApp(ctk.CTk):
    ...
    def _browser_focus_prev(self):
        if self._browser_send_key("shift+Tab"):
            self.toast.show("Browser: previous item", 700)

    def _browser_focus_next(self):
        if self._browser_send_key("Tab"):
            self.toast.show("Browser: next item", 700)
    ...
    def _browser_show_type_keyboard(self):
        if not self._browser_can_remote_control():
            self.toast.show("Browser control unavailable")
            return
        self._browser_typing_mode = True
        self._touch_keyboard.show()
        self.toast.show("Browser typing mode", 900)
