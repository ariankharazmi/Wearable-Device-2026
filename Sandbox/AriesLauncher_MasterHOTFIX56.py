...
class TouchKeyboard:
    ...
    def _tokens(self):
        ...
        if getattr(self.app, "current_view", "") == "browser" and getattr(self.app, "_browser_typing_mode", False):
            # ← → at the FRONT so they're immediately selected when keyboard opens
            base = [self.TOK_BROWSER_BACK, self.TOK_BROWSER_FORWARD] + base
    ...
    def show(self):
        ...
        # In browser mode: pin the Tk window above Chromium using -topmost.
        # The main Tk window is transparent (bg=#000000), so browser content
        # shows through — only the keyboard panel itself is opaque.
        if getattr(self.app, "current_view", "") == "browser":
            try:
    ...
    def hide(self):
        ...
        self.visible = False
        # Release topmost so Chromium can come back to front
        try:
...
class VAApp(ctk.CTk):
    ...
    def _handle_rotary_click(self):
        ...
        if double:
            self._last_rotary_click_at = 0.0
            # Double-click: summon keyboard in browser mode,
            # or show touch keyboard for any active text input
    ...
    def show_camera(self):
        ...
        self._cam_label = ctk.CTkLabel(p, text="Initializing …",
                                        font=(FONT, 12 if (WIDTH <= 640 or HEIGHT <= 400) else 13), text_color=TXTD)
        self._cam_label.pack(fill="both", expand=True)

        bb = ctk.CTkFrame(self._vf, fg_color="transparent")
        bb.pack(pady=(0, 4 if (WIDTH <= 640 or HEIGHT <= 400) else 8))
        self._btn(bb, "Capture", self._capture_photo, True, 54 if (WIDTH <= 640 or HEIGHT <= 400) else None)
        self._btn(bb, "Restart", self._restart_cam, w=46 if (WIDTH <= 640 or HEIGHT <= 400) else None)
        self._btn(bb, "Stop", self._stop_camera, w=38 if (WIDTH <= 640 or HEIGHT <= 400) else None)
    ...
    def _cam_tick(self):
        ...
        try:
            ...
            if ret:
                ...
                preview_size = (390, 160) if (WIDTH <= 640 or HEIGHT <= 400) else (800, 450)
    ...
    def _browser_show_type_keyboard(self):
        # Always show the keyboard — ← → navigation works without xdotool.
        # Typing and element-focus require xdotool; warn if missing.
        if not shutil.which("xdotool"):
            self.toast.show("Tip: install xdotool for full browser control", 2000)
        self._browser_typing_mode = True
        # Bypass the active_input guard — browser keyboard has no input widget.
        self._touch_keyboard.ensure()
        self._touch_keyboard.visible = True
        self._touch_keyboard.sel_row = 0
        self._touch_keyboard.sel_col = 0   # start on ← (first token in browser mode)
        self._touch_keyboard._viewport_start = 0
        self._touch_keyboard._position(force=True)
        self._touch_keyboard.window.lift()
        self._touch_keyboard.window.tkraise()
        try:
            self.attributes("-topmost", True)   # float keyboard above Chromium
        except Exception:
            pass
        self._touch_keyboard.refresh()
        self.toast.show("← → navigate  Enter=open  2×click=close keyboard", 2200)
    ...
    def show_browser(self, search_query=None):
        ...
        compact = WIDTH <= 640 or HEIGHT <= 400
        overlay_h = 92 if compact else 156
        top_h = 28 if compact else 44
        ...
        top.pack(fill="x")
        top.pack_propagate(False)
        ctk.CTkLabel(top, text="‹  Browser", font=(FONT, 11 if compact else 16), text_color=C).pack(side="left", padx=8 if compact else 18, pady=5 if compact else 10)
        ctk.CTkLabel(top, text="Long press exits", font=(FONT, 10 if compact else 10), text_color="#48484A").pack(side="right", padx=8 if compact else 18, pady=5 if compact else 10)
        ctk.CTkFrame(self._vf, fg_color="#2C2C2E", height=1).pack(fill="x", padx=0)
        ...
        nav1 = ctk.CTkFrame(shell, fg_color="transparent")
        nav1.pack(fill="x", padx=4 if compact else 8, pady=(3 if compact else 8, 1))
        nav2 = ctk.CTkFrame(shell, fg_color="transparent")
        nav2.pack(fill="x", padx=4 if compact else 8, pady=(0, 2 if compact else 6))

        self._b_url = tk.StringVar(value=(search_query or "").strip())
        ent = ctk.CTkEntry(
            nav1,
            textvariable=self._b_url,
            placeholder_text="Search or URL...",
            font=(FONT, 11 if compact else 13),
            fg_color=PNL,
            text_color=TXT,
            border_color=C,
            height=22 if compact else 34,
        )
        ent.pack(side="left", fill="x", expand=True, padx=(0, 2 if compact else 6))
        ...
        if compact:
            self._btn(nav1, "Open", go, True, 34)
            self._btn(nav1, "⌨", self._browser_show_type_keyboard, w=24)
            self._btn(nav1, "←", self._browser_go_back, w=24)
            self._btn(nav1, "→", self._browser_go_forward, w=24)
            self._btn(nav2, "Home", self._browser_open_home, w=34)
            self._btn(nav2, "Close", lambda: (self._close_browser(), self._go_home()), w=34)
        else:
            self._btn(nav1, "Open", go, True, 62)
            self._btn(nav1, "⌨", self._browser_show_type_keyboard, w=46)
            self._btn(nav1, "←", self._browser_go_back, w=46)
            self._btn(nav1, "→", self._browser_go_forward, w=46)
            self._btn(nav1, "Home", self._browser_open_home, w=62)
            self._btn(nav1, "Close", lambda: (self._close_browser(), self._go_home()), w=62)

        help_row = ctk.CTkLabel(
            shell,
            text=("Rotate=focus · Click=open · 2×=keyboard (← →)" if compact else "Rotate = Prev/Next element  ·  Click = activate  ·  Double-click = keyboard with ← →"),
            font=(FONT, 8 if compact else 10),
            text_color=TXTD,
            justify="center",
        )
        help_row.pack(pady=(0, 2 if compact else 6))

        ent.bind("<Return>", go)
        self._register_text_input(ent, go)
        self.after(80, self._focus_root)
        if search_query:
            self.after(180, go)
    ...
    def show_translate(self):
        ...
        ctk.CTkOptionMenu(top, values=langs, variable=lv, fg_color=PNL,
                           ...
                           text_color=TXT,
                           command=self._set_target_language).pack(side="left")
    ...
    def show_settings(self):
        ...
        self._btn(row1, "Light", lambda: self._set_system_light_mode(True), w=48 if compact else 120)
        self._btn(row1, "Dark", lambda: self._set_system_light_mode(False), w=48 if compact else 120)
        ...
        self._btn(brow, "Bri-", lambda: (self._adjust_alpha(-0.05), self.after(80, self._refresh_settings_status)), w=42 if compact else 120)
        self._btn(brow, "Bri+", lambda: (self._adjust_alpha(+0.05), self.after(80, self._refresh_settings_status)), w=42 if compact else 120)
        ...
        self._btn(vrow, "Vol-", lambda: (self._adjust_volume(-1), self.after(120, self._refresh_settings_status)), w=42 if compact else 100)
        self._btn(vrow, "Vol+", lambda: (self._adjust_volume(+1), self.after(120, self._refresh_settings_status)), w=42 if compact else 100)
