...
class VAApp(ctk.CTk):
    ...
    def _touchpad_move(self, x, y):
        # ── Home screen: map touch X position directly to app index ──────────
        # This gives Apple TV / iPod-style continuous feel instead of
        # discrete swipe steps. Finger position on the pad = position in
        # the app rail. Moving left/right smoothly scrolls the cover flow.
    ...
    # Active X coordinate range from Cirque spec (128–1920)
    _TOUCH_X_MIN = 128
    _TOUCH_X_MAX = 1920

    def _home_touch_track(self, x, y):
        """Map absolute X position on trackpad to CoverFlow app index.

        The full pad width maps to the full app list. Moving your finger
        from left to right sweeps through all apps smoothly — like an
        iPod click wheel in linear mode. No swipe gestures needed.
        """
        n = len(self.cflow.apps)
        if n == 0:
            return

        # Clamp to active range
        x_clamped = max(self._TOUCH_X_MIN, min(self._TOUCH_X_MAX, x))

        # Map 0.0–1.0 across the pad to 0–(n-1) app index
        t = (x_clamped - self._TOUCH_X_MIN) / (self._TOUCH_X_MAX - self._TOUCH_X_MIN)
        target_idx = int(round(t * (n - 1)))
        target_idx = max(0, min(n - 1, target_idx))

        if target_idx != self.cflow.sel:
            self.cflow.sel = target_idx
            # Record so tap guard knows we moved (prevents accidental open)
            self._last_touchpad_swipe_at = time.perf_counter()
