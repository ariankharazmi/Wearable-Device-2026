...
class TouchpadController:
    ...
    Notes:
      - The Cirque on Radxa behaves better when treated like a continuous contact stream,
        not a single-sample gesture detector.
      - We therefore smooth small jitter, ignore huge bogus jumps, and accumulate travel
        across a touch before emitting swipe events.
    """

    POLL_SEC = 0.015
    TAP_WINDOW_SEC = 0.28
    TAP_MAX_TRAVEL = 28
    RELEASE_TIMEOUT_SEC = 0.09

    JITTER_FLOOR = 2
    JUMP_THRESHOLD = 1400

    SWIPE_AXIS_THRESHOLD = 90
    SWIPE_CROSS_AXIS_LIMIT = 85
    REPEAT_SWIPE_THRESHOLD = 150
    SWIPE_DEBOUNCE_SEC = 0.055
    ...
    def _begin_touch(self, x, y, now):
        ...
        self._smooth_x, self._smooth_y = float(x), float(y)
    ...
    def _emit_swipe_if_ready(self, now):
        if now - self._last_swipe_at < self.SWIPE_DEBOUNCE_SEC:
            return

        ax = abs(self._accum_dx)
        ay = abs(self._accum_dy)

        if ax >= self.SWIPE_AXIS_THRESHOLD and ay <= self.SWIPE_CROSS_AXIS_LIMIT:
            direction = "right" if self._accum_dx > 0 else "left"
        ...
        if ay >= self.SWIPE_AXIS_THRESHOLD and ax <= self.SWIPE_CROSS_AXIS_LIMIT:
            direction = "down" if self._accum_dy > 0 else "up"
            self._last_swipe_at = now
            self._swiped_this_touch = True
            self._accum_dx = 0.0
            self._accum_dy = 0.0
        ...
        # sustained motion: allow another swipe while still touching
        if ax >= self.REPEAT_SWIPE_THRESHOLD:
            direction = "right" if self._accum_dx > 0 else "left"
            self._last_swipe_at = now
            self._swiped_this_touch = True
            self._accum_dx = 0.0
        ...
        if ay >= self.REPEAT_SWIPE_THRESHOLD:
            direction = "down" if self._accum_dy > 0 else "up"
            self._last_swipe_at = now
            self._swiped_this_touch = True
            self._accum_dx = 0.0
    ...
    def _handle_touch_point(self, x, y, now):
        ...
        raw_dx = x - self._last_x
        raw_dy = y - self._last_y

        # Ignore obvious bogus packets without killing the current touch
        if abs(raw_dx) > self.JUMP_THRESHOLD or abs(raw_dy) > self.JUMP_THRESHOLD:
            self._last_contact_at = now
            return

        # Light smoothing makes the pad feel less twitchy
        self._smooth_x = (0.65 * self._smooth_x) + (0.35 * x)
        self._smooth_y = (0.65 * self._smooth_y) + (0.35 * y)
        ...
        # Keep contact alive even if movement is tiny
        self._last_contact_at = now

        if abs(dx) <= self.JITTER_FLOOR and abs(dy) <= self.JITTER_FLOOR:
            return

        total_dx = sx - self._start_x
        total_dy = sy - self._start_y
        self._max_travel = max(self._max_travel, abs(total_dx), abs(total_dy))

        self._accum_dx += dx
        self._accum_dy += dy
        self._emit_swipe_if_ready(now)
    ...
    def _finish_touch_if_needed(self, now):
        ...
        touch_age = now - self._touch_started
        if (
            not self._swiped_this_touch
            and touch_age <= self.TAP_WINDOW_SEC
