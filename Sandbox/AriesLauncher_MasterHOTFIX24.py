...
class VAApp(ctk.CTk):
    ...
    _KEYBOARD_SHOW_SETTLE_SEC = 0.30
    _KEYBOARD_TAP_SETTLE_SEC = 0.36
    _KEYBOARD_NOISE_FLOOR = 16
    _KEYBOARD_DRAG_START_PX = 520.0
    _KEYBOARD_STEP_PX = 520.0

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 520.0
    _HOME_DRAG_START_PX = 360.0
    _HOME_ANCHOR_RESET_SEC = 0.14
    _HOME_NOISE_FLOOR = 24
    _HOME_HOLD_FREEZE_SEC = 0.10
    _HOME_IDLE_FREEZE_SEC = 0.04
    ...
    def _touchpad_tap(self):
        ...
        if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.35:
            return
        if (now - getattr(self, "_last_touchpad_motion_at", 0.0)) < 0.22:
            return
