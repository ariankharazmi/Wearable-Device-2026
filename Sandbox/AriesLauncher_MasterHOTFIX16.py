...
_ICON_FILES = [
    ...
    "spatialaudio.png", "translate.png", "video.png", "browser.png", "power.png",
]
...
class TouchpadController:
    ...
    MOVE_THRESHOLD = 4
    TAP_MAX_TRAVEL = 90             # ~2mm max travel for tap (was 12 = 0.25mm, too tight)
    ...
    STABLE_READS_FOR_MOVE = 3      # Require 3 consecutive stable reads before emitting move
...
class VAApp(ctk.CTk):
    ...
    def _touchpad_move(self, x, y):
        ...
        try:
            ...
            moved = False
            while self._keyboard_drag_accum >= step_px:
                self._keyboard_drag_accum -= step_px
    ...
    # Units per app step on the home screen (~330 units ≈ 7mm on a 38mm pad).
    # Tune this up if one swipe skips too many apps; down if it takes too much effort.
    _HOME_STEP_PX = 330
    # If no move event arrives for this long, the finger lifted → reset anchor.
    _HOME_ANCHOR_RESET_SEC = 0.18

    def _home_touch_track(self, x, y):
        """Apple TV style: relative delta from touch anchor, one app per ~7mm.

        Swiping left/right from anywhere on the pad moves the carousel by
        exactly as many apps as the finger traveled in multiples of _HOME_STEP_PX.
        A short deliberate flick = 1 app. A longer sweep = more apps.
        The anchor resets each time the finger lifts so the next touch starts fresh.
        """
        now = time.perf_counter()

        # Detect new touch: if anchor is stale (finger was lifted), reset it.
        if (self._home_drag_anchor_x is None
                or (now - self._home_drag_last_move_at) > self._HOME_ANCHOR_RESET_SEC):
            self._home_drag_anchor_x = x
            self._home_drag_accum = 0.0

        self._home_drag_last_move_at = now

        dx = x - self._home_drag_anchor_x

        moved = False
        while dx >= self._HOME_STEP_PX:
            self.cflow.move(+1)
            self._home_drag_anchor_x += self._HOME_STEP_PX
            dx -= self._HOME_STEP_PX
            moved = True
        while dx <= -self._HOME_STEP_PX:
            self.cflow.move(-1)
            self._home_drag_anchor_x -= self._HOME_STEP_PX
    ...
    def _touchpad_tap(self):
        self._home_drag_anchor_x = None   # finger lifted — next touch starts fresh
        if (time.perf_counter() - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.30:
            return
