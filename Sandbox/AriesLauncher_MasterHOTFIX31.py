...
class VAApp(ctk.CTk):
    ...
    _KEYBOARD_DRAG_START_PX = 180.0    # stronger startup deadzone so taps do not become rail drags
    _KEYBOARD_STEP_PX = 420.0          # much slower letter stepping; deliberate drag required

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 1200.0             # extra slow app stepping; normal swipe should stay near one app
    _HOME_DRAG_START_PX = 220.0        # require a clearly intentional swipe before moving apps
    ...
    def _touchpad_tap(self):
        ...
        if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.14:
            return
    ...
    def _camera_candidate_indices(self):
        ...
        ordered = []
        for idx in preferred + sorted(dict.fromkeys(indices)):
    ...
    def _open_camera_capture(self, idx):
        ...
        for backend in backends:
            cap = None
            try:
                cap = cv2.VideoCapture(idx) if backend in (None, getattr(cv2, 'CAP_ANY', None)) else cv2.VideoCapture(idx, backend)
