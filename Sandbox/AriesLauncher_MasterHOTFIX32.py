...
class VAApp(ctk.CTk):
    ...
    _KEYBOARD_SHOW_SETTLE_SEC = 0.20
    _KEYBOARD_TAP_SETTLE_SEC = 0.16
    _KEYBOARD_NOISE_FLOOR = 16
    _KEYBOARD_DRAG_START_PX = 260.0    # stronger startup deadzone so taps do not become rail drags
    _KEYBOARD_STEP_PX = 620.0          # much slower letter stepping; deliberate drag required

    # Home-screen drag tuned for deliberate Apple-TV-like movement.
    _HOME_STEP_PX = 1800.0             # extra slow app stepping; normal swipe should stay near one app
    _HOME_DRAG_START_PX = 360.0        # require a clearly intentional swipe before moving apps
    ...
    def _touchpad_tap(self):
        ...
        if (now - getattr(self, "_last_touchpad_swipe_at", 0.0)) < 0.24:
            return
