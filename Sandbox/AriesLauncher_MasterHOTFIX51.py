...
class TouchKeyboard:
    ...
    def _position(self, force=False):
        ...
        abs_h = 132 if (WIDTH <= 640 or HEIGHT <= 400) else (146 if app_h <= 420 else 156)
...
class VAApp(ctk.CTk):
    ...
    def _cam_tick(self):
        ...
        try:
            ...
            if ret:
                ...
                preview_size = (560, 315) if (WIDTH <= 640 or HEIGHT <= 400) else (800, 450)
    ...
    def show_photos(self):
        ...
        preview_w, preview_h = ((560, 210) if (WIDTH <= 640 or HEIGHT <= 400) else (520, 280))
        ...
        scroll = ctk.CTkScrollableFrame(p, fg_color="transparent",
                                         scrollbar_button_color="#48484A", height=(92 if (WIDTH <= 640 or HEIGHT <= 400) else 130))
        ...
        for i, fname in enumerate(files):
            ...
            try:
                thumb_size = (96, 60) if (WIDTH <= 640 or HEIGHT <= 400) else (130, 82)
                thumb_btn = (104, 68) if (WIDTH <= 640 or HEIGHT <= 400) else (138, 90)
    ...
    def show_browser(self, search_query=None):
        ...
        overlay_h = 122 if compact else 146
        top_h = 38 if compact else 44
        pad_x = 8 if compact else 10
        font_main = 13 if compact else 16
        font_help = 9 if compact else 10
        entry_h = 30 if compact else 34
        btn_open_w = 54 if compact else 62
        btn_small_w = 38 if compact else 46
        btn_mid_w = 54 if compact else 62
        ...
        top.pack(fill="x")
        top.pack_propagate(False)
        ctk.CTkLabel(top, text="‹  Browser", font=(FONT, font_main), text_color=C).pack(side="left", padx=14 if compact else 18, pady=8 if compact else 10)
        ctk.CTkLabel(top, text="Long press exits", font=(FONT, 9 if compact else 10), text_color="#48484A").pack(side="right", padx=14 if compact else 18, pady=8 if compact else 10)
        ctk.CTkFrame(self._vf, fg_color="#2C2C2E", height=1).pack(fill="x", padx=0)

        shell = ctk.CTkFrame(self._vf, corner_radius=10 if compact else 12, fg_color=PNL)
        shell.pack(fill="both", expand=True, padx=8 if compact else 10, pady=(4 if compact else 6, 4 if compact else 6))

        nav = ctk.CTkFrame(shell, fg_color="transparent")
        nav.pack(fill="x", padx=pad_x, pady=(6 if compact else 8, 4 if compact else 6))
        ...
        ent.pack(side="left", fill="x", expand=True, padx=(0, 4 if compact else 6))
        self._browser_entry = ent

        def go(_=None):
            self._browser_launch(self._b_url.get())

        self._btn(nav, "Open", go, True, btn_open_w)
        self._btn(nav, "⌨", self._browser_show_type_keyboard, w=btn_small_w)
        self._btn(nav, "←", self._browser_go_back, w=btn_small_w)
        self._btn(nav, "→", self._browser_go_forward, w=btn_small_w)
        self._btn(nav, "Home", self._browser_open_home, w=btn_mid_w)
        self._btn(nav, "Close", lambda: (self._close_browser(), self._go_home()), w=btn_mid_w)

        help_row = ctk.CTkLabel(
            shell,
            text="Rotate = Prev/Next focus  ·  Click = activate  ·  Double-click = keyboard",
            font=(FONT, font_help),
            text_color=TXTD,
            justify="center",
        )
        help_row.pack(pady=(0, 4 if compact else 6))
    ...
    def show_settings(self):
        ...
        compact = WIDTH <= 640 or HEIGHT <= 400
        padx = 12 if compact else 20
        pady = 8 if compact else 12
        bw = 94 if compact else 120
        vw = 82 if compact else 100
        micro_h = 60 if compact else 72
        ...
        row1.pack(fill="x", padx=padx, pady=(10, 6))
        self._btn(row1, "SystemLight", lambda: self._set_system_light_mode(True), w=bw)
        self._btn(row1, "SystemDark", lambda: self._set_system_light_mode(False), w=bw)

        ctk.CTkLabel(p, textvariable=self._settings_brightness_var,
                      font=(FONT, 12 if compact else 13, "bold"), text_color=TXT).pack(anchor="w", padx=padx, pady=(pady, 4))
        brow = ctk.CTkFrame(p, fg_color="transparent")
        brow.pack(fill="x", padx=padx, pady=(0, 6))
        self._btn(brow, "Brightness-", lambda: (self._adjust_alpha(-0.05), self.after(80, self._refresh_settings_status)), w=110 if compact else 120)
        self._btn(brow, "Brightness+", lambda: (self._adjust_alpha(+0.05), self.after(80, self._refresh_settings_status)), w=110 if compact else 120)

        ctk.CTkLabel(p, textvariable=self._settings_volume_var,
                      font=(FONT, 12 if compact else 13, "bold"), text_color=TXT).pack(anchor="w", padx=padx, pady=(pady, 4))
        vrow = ctk.CTkFrame(p, fg_color="transparent")
        vrow.pack(fill="x", padx=padx, pady=(0, 6))
        self._btn(vrow, "Volume-", lambda: (self._adjust_volume(-1), self.after(120, self._refresh_settings_status)), w=vw)
        self._btn(vrow, "Volume+", lambda: (self._adjust_volume(+1), self.after(120, self._refresh_settings_status)), w=vw)

        ctk.CTkFrame(p, fg_color="#2C2C2E", height=1).pack(fill="x", padx=padx, pady=(10, 4))
        ...
        mic_box = ctk.CTkTextbox(p, height=micro_h, font=(FONT, 10 if compact else 11),
                                  fg_color="#1C1C1E", text_color=TXT)
        mic_box.pack(fill="x", padx=padx, pady=(4, 4))
        ...
        if self.rotary.available:
            ctk.CTkFrame(p, fg_color="#2C2C2E", height=1).pack(fill="x", padx=padx, pady=(6, 4))
